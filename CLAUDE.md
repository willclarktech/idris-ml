...
data Layer : (inputSize : Nat) -> (outputSize : Nat) -> Type -> Type where
  ...
  LstmLayer : Matrix (4 * outputSize) inputSize ty -> Matrix (4 * outputSize) outputSize ty -> Vector (4 * outputSize) ty -> Vector outputSize ty -> Vector outputSize ty -> Maybe AnyPtr -> Maybe AnyPtr -> Maybe AnyPtr -> Layer inputSize outputSize ty
...
- **Dense optimizer**: `DenseOptimizer`/`DenseOptimizerState` in Optimizer.idr use C arrays indexed by integer pid_id instead of `SortedMap String Double`. `collectGradsDense` accumulates gradients into a pre-allocated C array during backward (no per-result FFI calls, no SortedMap inserts). The gradient array is persistent across epochs via `grad_alloc_reuse` (calloc once, memset-zero on reuse). Optimizer step functions (`rmsprop_vc_step`, `sgd_step`, `adam_gc_step`) operate in-place on the array. `applyDeltasDense` uses the Scheme `pid-to-id` hash table for O(1) lookup. NTM examples use this path via `epochTwoPhaseDense`; supervised/LSTM examples still use the original `SortedMap` path. Must call `getNumPids 0` after `autoName` to get the parameter count for `initDenseState`
