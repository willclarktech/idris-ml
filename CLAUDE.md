...
- **Target networks — two patterns**: DQN uses a `toDoubleNetwork` snapshot refreshed every N steps (hard sync, simple, fast) — the target is a frozen `Network i hs o Double` with no autograd graph. SAC uses Variable-CPU target networks registered under their own scope (`q1tgt_` / `q2tgt_`) with no optimizer owning them, and polyak-blends every step via `polyakBlend tau "q1_" "q1tgt_"` (→ C FFI `polyak_blend` in all three backends). The registry-level FFI approach avoids having to write an `emap2` Network walker, which would hit `AnyLayer` type-erasure issues (two existentials, no proof the concrete layer types match). Forward through a Variable target net still creates tape entries, but since no optimizer owns the target scope, leaked gradients are ignored.
