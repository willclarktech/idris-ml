...
- **Periodic GC for long training**: NTM training (50K+ epochs) OOMs without periodic forced GC. `forceGC` (exported from Variable.idr) calls Chez `(collect)` every 10 epochs in NTM training loops. The FFI lambda must take 0 args — `%World` is erased in Chez Scheme's PrimIO calling convention
