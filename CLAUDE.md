...
- **Imports**: three blank-line-separated tiers, each alphabetical — (1) external (stdlib + compiler API + curated deps), (2) cross-package libraries (`Ml.*` / `Gym.*` / `Transformers.*` from wherever the file sits), (3) package-local modules. `make fmt` enforces this via idris-fmt's import-sort pass; "local" is derived per file from the owning ipkg's `modules` list (`Format.Ipkg`), so the general-purpose tool hardcodes no repo names and there is nothing to configure.
