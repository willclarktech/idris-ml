...
- **Performance documentation regime**: four files, each with a distinct role. Don't conflate them; updating the wrong one loses information.
  - **`docs/develop/perf-log.jsonl`** — *append-only* raw measurement log. One JSON object per line, one line per measurement. Both `scripts/perf-run.sh` (full-convergence runs, `kind: "run"`) and `scripts/perf-baseline.sh` (two-point ms/epoch ratios, `kind: "baseline"`) auto-append. Always tagged with the short commit hash (`+dirty` if uncommitted changes). **Never edit or delete prior entries** — historical numbers are regression evidence and avoid re-running expensive measurements. If a measurement is later determined to be invalid, append a follow-up entry that says so. Query via `jq` (cookbook examples in `perf-log.md`).
  - **`docs/develop/perf-log.md`** — schema documentation for `perf-log.jsonl`: field definitions, conventions, a `jq` cookbook, plus the pre-2026-05-09 markdown-format entries archived at the bottom for historical record. Don't add new entries here — they go to the JSONL.
  - **`docs/develop/perf-baseline.md`** — *current-state* table. Human-readable snapshot of every example × backend ratio at the latest commit. Re-written, not appended. The summary view; raw measurements are in the JSONL. When you re-measure cells that moved, update this table.
  - **`docs/develop/perf-changes.md`** — *append-only* log of every perf *change* we make. One entry per change: motivation (what the profile showed, what the hypothesis was), the change itself (file + key function), impact (before/after ms-per-epoch and ratios on affected cells, with measurement-noise call-outs), commit hash, date, outcome (landed / reverted / partial). Reverted attempts stay — the negative result saves future redoing.
