...
| DNC-recall batch>=2 SIGSEGV on tape | M | `./build/exec/dnc-recall --epochs 1 --max-items 2 --batch 2 --seed 42` crashes with real SIGSEGV caught by idrisml_sigsegv_handler: backtrace `binop_elementwise + 196` at fault address in high heap territory (varies per run, e.g. `0xd40800000`). batch=1 works fine. Different bug from the DNC-copy realloc issue (which was spurious Chez-level); this one's a genuine bad pointer dereferenced inside an elementwise op during forward. Suspect: stale arena or tape pointer specific to DNC's scatter-based read path. DNC-recall is currently excluded from `test-examples` (see Makefile EXAMPLES note) until fixed |
| DNC in test-examples | S–M | Both DNC examples (`example-dnc-copy`, `example-dnc-recall`) are currently *excluded* from `test-examples`. dnc-copy at defaults (max-len=20, batch=16) takes ~4 min per epoch — even aggressive caps like `--max-len 3 --batch 4 --epochs 20` run long and sometimes time out. dnc-recall is blocked on the batch>=2 SIGSEGV above. Once that's fixed, pick viable caps (probably `--max-len 3 --batch 1` for both) and re-add to EXAMPLES so we catch regressions. Alternatively add a separate slow target (`test-examples-slow`) run on a cadence that tolerates longer wall times |
...
- Tape backend DNC-copy SIGSEGV: DNC-copy at batch=16 was crashing with `"Exception: invalid memory reference. Some debugging context lost"`. Initial commit 13ca64d worked around it by pre-sizing the tape buffer to 2M entries so `realloc()` never ran — at the time I attributed the crash to a memory-corruption bug around ≥ 2 tape reallocs. Deeper investigation disproved that: setting SIGSEGV to `SIG_DFL` runs the same workload cleanly with no coredump, and a custom handler in the dylib never fires. There's no real C-side memory fault. Chez Scheme's own SIGSEGV handler (likely part of its GC write-barrier / safepoint mechanism) appears to misinterpret some legitimate FFI access pattern as a fault once the workload gets large enough. Commit 42b9e44 overrides Chez's handler via `__attribute__((constructor))` — clean runs, no workaround needed, `TAPE_INIT_CAP` restored to 4096
