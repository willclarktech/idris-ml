...
| Pinpoint real root cause of tape-realloc SIGSEGV | M | The 2M-entry pre-size of `TAPE_INIT_CAP` (commit 13ca64d, restored after commit 42b9e44 was reverted) is a workaround, not a fix. Reality: multi-epoch DNC on tape exposes a real SIGSEGV whenever the tape `realloc()`s ≥ 2 times during training. A previous attempt (42b9e44) concluded the signal was spurious from Chez's handler — but that was based on 1-epoch testing. At 3+ epochs the same workload coredumps cleanly under SIG_DFL, so the fault is real. ASAN can't attach to Chez on macOS. Plausible next diagnostic: build a custom Chez with ASAN enabled, or write a C-only harness replaying the FFI sequence. 2M covers all current test-examples workloads; larger batch/seq workloads (DNC at batch=16 default) still crash |
| DNC in test-examples | S–M | Both DNC examples (`example-dnc-copy`, `example-dnc-recall`) are *excluded* from `test-examples` because (a) DNC is genuinely slow per epoch on tape — ~3-4s/epoch even at `--batch 1 --max-len 3`, so a meaningful smoke (≥ 30 epochs to see loss actually moving) is > 2 min per (example, backend); (b) at smoke epoch counts accuracy barely creeps above random (0.51-0.52), so any threshold above "didn't diverge" is flaky. Once the tape-realloc SIGSEGV root cause is fixed, consider adding DNC back at `--batch 1 --max-len 3 --epochs 30` with a very loose threshold (≥ 0.40, catches NaN/divergence only), or move DNC to a separate slow target run on a cadence that tolerates longer wall times |
