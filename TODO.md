...
| RL deep algorithms: A2C / PPO / SAC (Idris) | M | Round 2 of the RL suite. A2C PyTorch reference already ships and converges to 165 on CartPole; Idris port hit Idris-2 parser/visibility issues (reserved-keyword `total` + private `autoNameNetwork`) and was deferred. Once Idris-side issues resolved, follow up with PPO (clipped surrogate on Pendulum using `RL.Gae`) and SAC (stochastic Gaussian actor + twin Q nets on Pendulum using `RL.ReplayBuffer`). PyTorch refs for PPO / SAC also needed |
...
- `Train.runTrainingIO` — IO-based epoch-function variant of `runTraining` for examples that need sampling / RNG threading inside the epoch (used by DQN; pure `runTraining` now thin-wraps it)
- PyTorch references: `torch_ref/models/{q_learning,sarsa,monte_carlo,dqn,a2c}.py` + tests under `correctness/`, all passing
