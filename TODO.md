...
| RL algorithms beyond REINFORCE | M–L | `idris-gym` now has feature-complete Gymnasium API (Env/Space/Info/Outcome + wrappers + vector env) and Classic Control + Toy Text envs. Add algorithms: PPO (continuous control via Pendulum / MountainCarCont), A2C, DQN (discrete via MountainCar / CartPole), tabular Q-learning (FrozenLake / CliffWalking). `Example/Reinforce.idr` remains the only RL example currently |
