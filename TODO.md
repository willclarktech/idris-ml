...
| Convergence-regression coverage gap | M | After collapsing test-examples to a crash-only smoke gate (each example runs 3-10 epochs, thresholds at safety-net level), there's no automated check that catches "model still trains correctly" regressions on the supervised / recurrent / memory-aug examples. `test-examples-convergence` covers the RL subset at 3 seeds, but not NTM / DNC / Supervised / RNN / LSTM / Transformer / GPT / Mnist / SeqClassify. Need either: (a) extend `test-examples-convergence` to cover the supervised/recurrent set at single seed but full epochs, or (b) add a separate `test-examples-convergence-supervised` lane. The convergence target was already in this file as "NTM/DNC multi-seed convergence target" — bigger now since it covers everything not in RL convergence |
