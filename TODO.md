...
| RL environments and examples | M–L | Currently only CartPole with REINFORCE. Expand: (1) more environments — MountainCar, Acrobot, Pendulum (all pure Idris physics, no gym dependency); (2) more algorithms — PPO, A2C, DQN; (3) type-safe env interface (`Env state action reward` with compile-time dimension checking on observations/actions). CartPole already proves the pattern works (pure Idris env + tensor-level policy). Idris 2's dependent types could enforce action space bounds at compile time |
...
| Monorepo restructure | M–L | Reorganize into a monorepo with separate packages for different project types (library core, C backends, examples, PyTorch references, Jupyter kernel). Enables independent versioning, clearer dependency boundaries, and easier contribution to individual components. Investigate workspace/package tooling: Idris 2 `pack` supports multi-package repos, Python side already uses `uv`. Consider: `packages/core/` (library), `packages/backends/` (C code), `packages/examples/`, `packages/pytorch-ref/`, `packages/jupyter/` |
