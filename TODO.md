...
| Item | Difficulty | Notes |
|------|-----------|-------|
| Fix stale top-level `build/ttc/` cache for example rebuilds | S | When a library module (e.g. `Train.idr`) changes but an example source (`Example/*.idr`) doesn't, `make example-<name>` and `make check-examples` reuse the cached `build/ttc/Example/<Name>.ttc` from the previous build. That cached TTC was linked against the OLD library TTC, so runtime behavior silently reflects the old library code even after `install`. Observed while testing the RSS-logging standardization: Train.idr was updated, installed, and example-supervised "rebuilt", but the peak/cur suffix never appeared until I `rm -rf build/ttc`. This is a real correctness footgun — changes to shared library code can look broken at runtime when in fact they're correct. Options: (1) change the Makefile's example targets to `rm -rf build/ttc/Example/<slug>*` before compiling; (2) more nuclear: `rm -rf build/ttc` at the top of `install-core`; (3) investigate whether `idris2 -o <file>` has a flag to disable/bust cache for a single-file build. The `make test-examples` sweep is the most at-risk — it claims to validate the current code but may be validating yesterday's. Also flag in `docs/develop/gotchas.md` |
