...
| MNIST epoch-semantics alignment with PyTorch reference | M | Idris MNIST does 1-batch-per-epoch (single `genBatch` call → forward+backward+step). PyTorch's `train_epoch` iterates ALL batches (~1875 at batch=64) per epoch. So same epoch-count means a 1875× compute difference — masquerades as identical hyperparameters. Currently aligned by *value* (Idris epochs=2000 ≈ batches; converges to acc=0.92 at the 0.85 threshold) but not by *semantics*. Proper fix: refactor Idris MNIST training loop to iterate all batches per logical epoch (pattern: `runTrainingIO` with `dataSrc=pure ()` and the epoch function calls `genBatch` `batchesPerEpoch` times internally — see `Example/Dqn.idr` for prior art using the same `()` data-source pattern). Then drop epoch count to ~5–10 (≤ 1 PyTorch-equivalent epoch) so wall-time stays under the 4h CONVERGENCE_TIMEOUT. Also: same shape applies to seq-classify but it's less impactful (synthetic data, current 1000 epochs already matches PyTorch's 1000 single-batch epochs). See `docs/develop/reference-alignment.md` "MNIST epoch semantics" entry |
