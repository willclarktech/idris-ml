...
| RL algorithm suite (Q-learning, SARSA, MC, DQN, A2C, PPO, SAC) | L | `idris-gym` has feature-complete Gymnasium API (Env/Space/Info/Outcome + wrappers + vector env) and Classic Control + Toy Text envs. Ship in two rounds: (R1) tabular — Q-learning & SARSA on CliffWalking, first-visit MC on Blackjack; (R2) deep — DQN on CartPole/MountainCar, A2C on Acrobot, PPO + SAC on Pendulum. R2 requires two shared RL primitives: `RL.ReplayBuffer` (ring-buffer with IOArray backing, used by DQN + SAC) and `RL.Gae` (pure generalized advantage estimation, used by A2C + PPO). Each algorithm ships with a matching PyTorch reference under `packages/pytorch/torch_ref/models/` per the correctness-oracle convention |
