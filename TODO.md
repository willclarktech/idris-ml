...
| Pinpoint real root cause of tape-realloc SIGSEGV | M | The 2M-entry pre-size of `TAPE_INIT_CAP` is a workaround, not a fix. **Investigation (2026-04-24):** isolated the trigger to `free(old_tape)` in `tape_append`'s growth path. Evidence: (a) tape poisoning (`memset(old_tape, 0x11, ...)` before free) reproduces the crash at `binop_elementwise + 280`, writing to a data pointer; (b) disabling arena chunk reuse didn't help; (c) **leaking the old tape (skipping `free`) runs cleanly at 3+ epochs** — deterministic fix. So something holds a live pointer into the freed tape region that becomes stale after free(). No `TapeEntry*` captures exist in our C (grep-verified); the stale reader is hiding in (i) a tape field that's misused somewhere, (ii) Chez scheme-side caching a tape-adjacent pointer via FFI, or (iii) Apple malloc's large-allocation cache interacting with stale references. Alternative workaround worth switching to: `TAPE_INIT_CAP=4096` + leak-on-grow (same ~114 MB peak, lighter for small programs that never grow). Finding the stale reader is the remaining work |
...
| RL convergence target uses smoke thresholds | S | `test-examples-convergence` (RL multi-seed) reuses `test-examples.expect` for thresholds. After the smoke collapse those thresholds are safety-net (`>= -2500` for PPO/SAC, etc.) — too loose for a convergence lane. Migrate the RL convergence target to use `test-examples-convergence.expect` (with tightened RL thresholds restored to pre-collapse values) for proper convergence checking |
| NTM/DNC multi-seed convergence | M | `test-examples-convergence-memory` is the new manual lane (single seed=42, full default epochs, hours wall time). For *multi-seed* convergence on NTM/DNC — which is the canonical regression mode (gotchas.md:185 — seed=42 converges, seed=123 doesn't) — either run `test-examples-convergence-memory` 3-5 times with `--seed` overrides, or build a dedicated multi-seed target (very expensive). Lower priority since the manual single-seed lane is now sufficient for release validation |
| MLX backend stubs roadmap | M | `backend_mlx.cpp` has 14 STUB functions; none currently block any example (verified via test-examples passing on MLX). Categorized: (a) 5 functions implemented in tape/torch but with no Idris FFI binding (`tensor_cross_entropy`, `tensor_mse_loss`, `tensor_unbatch`, `tensor_grad`, `tensor_cat_from_array`) — could be implemented as MLX wrappers around mx ops to reach feature parity; only `tensor_grad` has a non-Idris caller (`scripts/test_cuda_colab.sh`). (b) 9 functions with Variable.idr FFI bindings but no example calls (`tensor_squeeze`, `tensor_stack`, `tensor_cat`, `tensor_batch`, `tensor_sum_dim`, `tensor_detach`, `tensor_with_grad`, `tensor_lstm_cell`, `tensor_lstm_gates`) — implement opportunistically when an example needs them. Each is a 10-50 line implementation following the existing MLX op pattern (mx call + tape_append for autograd replay). Torch backend has zero stubs; tape backend has all 14 implemented |
...
| DNC on MLX: state-Variable lifecycle bug | M | DNC's `DncState` holds structured `Matrix n n (Variable d)` (link matrix) and `Vector n (Variable d)` (usage, precedence, etc.). Each scalar Variable wraps a `Tensor*` allocated via `prim__createScalar` / `tensor_select` — non-persistent on MLX. `tape_reset` (called at end of every optimizer step) deletes all non-persistent tensors, leaving DncState's stored pointers dangling. Epoch 1's forward walks them and SIGSEGVs partway through. NTM has the same pattern (memory matrix) but smaller per-epoch heap churn → freed memory hasn't been reused at access time → "works" by luck. Tape backend isn't affected (different lifecycle). Currently dnc-copy/dnc-recall are skipped on the MLX row of test-examples until this is fixed. Likely fix shapes: (a) make the state Variables' Tensors persistent at init time (needs a new FFI variant of `prim__createScalar` that sets `persistent=1`), or (b) refactor DncState to carry a single tensor handle (`linkTensor` etc., already declared as `Maybe AnyPtr` fields) and only materialize scalar Variables transiently during forward |
