...
| Torch backend: GPT crashes in `tensor_cross_attention` on `masked_fill_` | S–M | After fixing the `tensor_alloc_ints` symbol export, `example-gpt [torch]` now loads but crashes with `masked_fill_ only supports boolean masks, but got mask with dtype double`. Stack trace bottoms out at `tensor_cross_attention + 260` in `libidrisml_torch.dylib`. The mask passed to `masked_fill_` is float64; torch requires `kBool`. Fix: locate the mask construction in `tensor_cross_attention` in `packages/backends/backend_torch.cpp`, convert to bool (`.to(torch::kBool)`) before calling `masked_fill_`. Verify: `make BACKEND=torch example-gpt`. MNIST and SeqClassify are clean on torch after the symbol fix |
