...
| Torch backend in CI | M | MLX shipped (see Done). Torch needs libtorch (~150 MB) installed on the runner — bigger setup than MLX, so defer to its own PR. test-examples already iterates `for b in tape mlx torch` and gracefully skips backends whose dylib failed to build, so adding torch is purely additive: install libtorch on a runner (Ubuntu has the easiest packaging via libtorch tarball), set `LIBTORCH_PATH`, done |
...
- GitHub Actions workflow: `.github/workflows/test.yml` matrix across `ubuntu-latest` + `macos-latest` (both free unlimited on public repos). Initially tape-only; MLX added on macos-latest via `python3 -m pip install --break-system-packages mlx` + `MLX_SITE` env export so test-examples' inner `for b in tape mlx torch` loop picks it up automatically. macOS lane now exercises both tape and MLX backends; ubuntu lane stays tape-only. Idris 2 v0.8.0 built from source with actions/cache
