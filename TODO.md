...
| Pinpoint real root cause of tape-realloc SIGSEGV | M | `fix(backend/tape): pre-size tape to 2M entries` (commit 13ca64d) is a workaround, not a fix — DNC-copy at default batch=16 crashed around idx ≈ 1.45M when the tape realloc'd its way up through several size classes. Bisection showed **≥ 2 reallocs** triggers the crash, 0 or 1 does not. Swapping `realloc` for malloc+memcpy+free reproduced it identically, so it's not a realloc-specific bug — something holds a live reference into the old tape buffer that gets corrupted on the second free. Five plausible hypotheses (backward capturing `&tape[i]`, arena-allocated Tensor aliasing the freed region, stack overflow in deep recursion, double-free of shared `op_meta`, Tensor self-reference via `tape_idx`) all failed to explain the pattern. ASAN can't attach to Chez Scheme on macOS (interceptors not installed via dlopen). The workaround costs 114 MB of virtual memory but ~0 physical RSS (lazy-commit mmap); acceptable for now. Plausible next steps: build a custom Chez with ASAN enabled, or write a C-only harness that replays the exact forward-pass sequence and sanitize that instead |
...
- Tape backend `TAPE_INIT_CAP` bump: DNC-copy at default batch=16 produces ~1.6M tape entries per batched forward, and with the old 4K initial capacity the tape would `realloc` ≥ 2 times and SIGSEGV (`"Exception: invalid memory reference"`) around idx 1.45M during forward. Bisection showed the crash triggers only when at least two growth events happen — swapping realloc for malloc+memcpy+free reproduces identically, so something holds a live reference into the old tape region across the free. Root cause still unknown (ASAN won't attach to Chez on macOS; tracked as a TODO). Workaround: pre-size to 2M entries (~114 MB virtual memory, lazy-committed, effectively free for small examples)
