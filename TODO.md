...
| Polyak soft target updates for SAC | S | SAC Idris uses hard target copy every N steps (τ=1) because we don't have a tensor-blend helper. Standard SAC uses τ=0.005 every step which is more stable. Add an FFI-level `tensor_polyak_update(target, online, tau)` op + Idris wrapper, then swap in. Should improve SAC convergence from -1200 to -250 band |
| Reparameterized actor gradient for SAC | S | SAC Idris currently uses log-prob-only actor gradient (`actorLoss = α·logπ - fromDouble minQ` where minQ is a pure Double) because our `forwardVarTensor` pipeline doesn't take a grad-tracked input tensor built from an obs-plus-sampled-action concatenation. PyTorch SAC uses the full reparameterized form where gradient flows to the actor both through `logπ` AND through `Q(s, â(θ_actor))`. The gap at 10k steps is ~650 on Pendulum greedy eval (-1973 Idris vs -1331 PyTorch). Add grad-tracked tensor concat + have Q-networks accept Variable-valued action inputs during the actor update |
