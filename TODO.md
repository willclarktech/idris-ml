...
| Item | Difficulty | Notes |
|------|-----------|-------|
| Extract test-data generation into its own package | S–M | `packages/idris-ml/src/Generate.idr` (`SequenceTask`, `copyTaskBinary`, `recallTaskBinary`, `randomBatchVect`, `patternData`, `copyTaskBinaryBatchVect`, etc.) is synthetic-data scaffolding used only by examples and tests — it doesn't belong in the core library. Extract it to a new package `idris-ml-data` (or similar) analogous to how `idris-gym` was split out. `idris-ml-examples` and `idris-ml-notebook` gain a dependency on the new package; `idris-ml` itself drops `Generate.idr` and its transitive deps (`DataPoint`, parts of `Sampler`) if unused elsewhere. Touch `DataLoader.idr` too — decide whether the generator-loader helpers belong alongside Generate. Benefits: smaller surface for the core lib, clearer separation between training primitives and task-specific data pipelines, faster recompiles for users who don't need synthetic generators |
