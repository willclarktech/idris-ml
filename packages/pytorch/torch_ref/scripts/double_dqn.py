"""Double DQN on CartPole training script with --lr-find support.

Output format and epoch semantics align with `Example.DoubleDqn` (Idris):
each "epoch" = one full episode + intra-episode replay updates, and the
"loss" reported to lr_find is `-episode_return` (matching the Idris
convention so the cross-backend lr_find comparison is meaningful).

Usage:
    python -m torch_ref.scripts.double_dqn [--lr 5e-4] [--epochs 300]
                                           [--seed 42] [--lr-find]
"""
...
def main() -> None:
    ...
    def epoch_fn() -> float:
        """One batched Double DQN episode. Returns -env-0_episode_return
        (matches Idris loss)."""
        new_step, ep_return, new_obs = double_dqn_episode_batched(
            vec_env,
            obs_state[0],
            q,
            target,
