...
def main() -> None:
    ...
    # Oracle run: take Idris' init weights, do exactly one epoch (which is one
    # optimizer step here — `generate` streams have no epoch length, so the
    # Idris loop takes a single step per epoch too), dump the result.
