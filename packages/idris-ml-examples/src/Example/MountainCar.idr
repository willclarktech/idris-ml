...
-- Batched epsilon-greedy across NumEnvs envs: one batched forward, then
-- per-env eps-vs-greedy. The explore gate is one `rng.uniform` per env and
-- the explored action one `rng.choice`, so a recorded run replays both.
...
epsGreedyBatched rng qB envs eps = go 0 envs
  where
    ...
    go i (_ :: rest) = do
      u <- rng.uniform
      a <- if u < eps
             then rng.choice [1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0]
             else do
...
-- its per-env episode clock, drawing the new state from MountainCar's own
-- U(-0.6, -0.4) start distribution. The clock mirrors Gymnasium's TimeLimit
-- — it counts from each env's own reset, and hitting it flags the
-- transition done (truncation) exactly as the reference's `term | trunc`
-- does. On MountainCar the cap is effectively the ONLY episode boundary
-- (random exploration never reaches the goal in 200 steps), so until
-- 2026-08-03 — no per-env clock, done=False finals — every single episode
-- boundary disagreed with the reference's.
stepAllAutoResetTrunc : Source -> Vect n MCState -> Vect n Nat -> Vect n Nat ->
                        (Vect n MCState, Vect n Double, Vect n Bool, Vect n Nat, Source)
  ...
  case mcStep s a of
    (r, s', outcome, _) =>
      let term     = done outcome
          ...
          (nextS, seedNext) = the (MCState, Source)
            (if isDone
               then reset {state=MCState} {action=Nat} {obs=Vect ObsDim Double} seed
               else (s', seed))
...
runEpisodeBatchedL opt (MkDqnState qNet target buffer envsRef stepRef slRef epsStart epsEnd epsDecay syncEvery batch gamma shaping onNames tgtNames rng nextSrc putSrc) = do
  ...
  pure1 (MkBang ret # MkDqnState qNet' target' buffer envsRef stepRef slRef epsStart epsEnd epsDecay syncEvery batch gamma shaping onNames tgtNames rng nextSrc putSrc)
  where
    go : (1 _ : QNet) -> (1 _ : QNet) -> Vect NumEnvs MCState -> Vect NumEnvs Nat -> Source -> Nat -> Double ->
         L IO {use = 1} (LPair (!* Double) (LPair QNet QNet))
    go qNet target _ _ seed Z ret = do
      let (venv, seedEnd) = the (VecEnv NumEnvs MCState, Source)
                                (resetAll {state=MCState} {action=Nat} {obs=Vect ObsDim Double} seed)
...
buildStateL cfg = do
  ...
  (rng, nextSrc, putSrc) <- liftIO1 (case cfg.replay of
                                       ...
                                       pth => mkReplay pth)
  src <- liftIO1 nextSrc
  let (initEnvs, src') = the (VecEnv NumEnvs MCState, Source)
                             (resetAll {state=MCState} {action=Nat} {obs=Vect 2 Double} src)
  liftIO1 (putSrc src')
...
discardStateL (MkDqnState qNet target _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) = do
  discard qNet
  discard target

finalReportL : Config -> Nat -> (1 _ : DqnState) -> L IO ()
finalReportL cfg epochsDone (MkDqnState qNet target _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) = do
