...
public export
record SwiGLU (hidden : Nat) (intermediate : Nat) (0 ex : Executor) (0 dt : DType) where
  constructor MkSwiGLU
  gateW : Tensor [intermediate, hidden] ex dt WithGrad
  upW   : Tensor [intermediate, hidden] ex dt WithGrad
  downW : Tensor [hidden, intermediate] ex dt WithGrad
...
swigluForward : {0 ex : Executor} -> Backend ex dt => {0 g : GradMode} -> {hidden, intermediate : Nat} ->
                SwiGLU hidden intermediate ex dt -> Tensor [hidden] ex dt g -> IO (Tensor [hidden] ex dt g)
swigluForward (MkSwiGLU gateW upW downW) input = do
  gate <- tmv (retypeGrad gateW) input
  up   <- tmv (retypeGrad upW)   input
  sg   <- tsilu gate
  mid  <- tmul sg up
  tmv (retypeGrad downW) mid
...
swiglu : {0 ex : Executor} -> Backend ex dt => {hidden, intermediate : Nat} ->
         Init (SwiGLU hidden intermediate ex dt)
swiglu = do
