...
mkVecDt xs = do
  raw <- ioRerun (\_ => bulkToTensor {ex=TestExecutor} {dt=dt}
...
bitlinearForwardOracle = do
  ...
  w <- tCreateTernaryPacked2d {ex=TestExecutor} {o=3} {i=4} bytesPtr byteCount
  ...
  y <- tBitlinearFwd {ex=TestExecutor} {cDt=TestDType} w s x b
  y0 <- readElem3 y 0
  y1 <- readElem3 y 1
  y2 <- readElem3 y 2
  let tol = 1.0e-6
...
bitlinearForwardOracleF32 = do
  ...
  y <- tBitlinearFwd {ex=TestExecutor} {cDt=F32} w s x b
  y0 <- readElem3Dt y 0
  y1 <- readElem3Dt y 1
  y2 <- readElem3Dt y 2
  let tol = 1.0e-5
...
mkMat34NoGrad xs = do
  raw <- ioRerun (\_ => bulkToTensor2d {ex=TestExecutor} {dt=TestDType}
...
absmeanQuantRoundtripOracle = do
  ...
  x <- mkVec       (the (Vect 4 Double) [1.0, 2.0, -0.5, 0.25])
  b <- mkVec       (the (Vect 3 Double) [0.1, -0.2, 0.3])
  y <- tBitlinearFwd {ex=TestExecutor} {cDt=TestDType} ternaryW scale x b
  y0 <- readElem3 y 0
  y1 <- readElem3 y 1
  y2 <- readElem3 y 2
  let tol = 1.0e-6
...
bitlinearHfPackedRoundtrip = do
  ...
  w <- tCreateTernaryFromHfPacked2d {ex=TestExecutor} {o=3} {i=4} bytesPtr
  s <- mkVecNoGrad (the (Vect 3 Double) [0.5, 0.25, 0.75])
  x <- mkVec       (the (Vect 4 Double) [1.0, 2.0, -0.5, 0.25])
  b <- mkVec       (the (Vect 3 Double) [0.1, -0.2, 0.3])
  y <- tBitlinearFwd {ex=TestExecutor} {cDt=TestDType} w s x b
...
bitlinearFwdHfQuantConsistency = do
  ...
  -- xq : Tensor [4] ex dt NoGrad; bias : WithGrad. Promote bias to NoGrad to match.
  bNoG <- weakenGrad {ex=TestExecutor} b
  yComposed <- tBitlinearFwd {ex=TestExecutor} {cDt=TestDType} w s xq bNoG

  -- Fused path: tBitlinearFwdHfQuant (useRmsNorm = False; rmsW is placeholder).
  xNoG <- weakenGrad {ex=TestExecutor} x
  yFused <- tBitlinearFwdHfQuant {ex=TestExecutor} {cDt=TestDType} w weightScale x b False xNoG 1.0e-5
