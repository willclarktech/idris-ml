...
mkInput2d xs =
  let rows = map (\r => VArray (map SArray r)) xs
      raw  = bulkToTensor2d {ex=TestExecutor} {dt=TestDType} {b=seq} {i=headDim} rows
      ...
      if r >= rEnd
        then pure (reverse acc)
          ...
          then go rEnd cEnd (r + 1) 0 acc
            ...
            let v = primItem2d {ex=TestExecutor} p r c
...
prop_rope_inverse_commutativity_body xs = do
  ...
  rotated  <- applyRope        {ex=TestExecutor} {seq=SEQ} {headDim=HEAD_DIM}
                               {maxPos=MAX_POS} tables 0 inputT
  restored <- applyRopeInverse {ex=TestExecutor} {seq=SEQ} {headDim=HEAD_DIM}
