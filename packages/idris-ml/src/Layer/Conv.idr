...
export
applyConv2D : {0 d : Device} -> UserDeviceLinear d => {inC, outC, h, w, kH, kW, padH, padW : Nat} ->
                Conv2DState inC outC h w kH kW padH padW
                              (inC * (h * w))
                              (outC * (ConvOutDim h kH padH * ConvOutDim w kW padW))
                              d g ->
...
export
applyConv2DBatched : {0 d : Device} -> UserDeviceLinear d => {inC, outC, h, w, kH, kW, padH, padW : Nat} -> {b : Nat} ->
                       Conv2DState inC outC h w kH kW padH padW
                                     (inC * (h * w))
                                     (outC * (ConvOutDim h kH padH * ConvOutDim w kW padW))
                                     d g ->
...
export
applyConv1D : {0 d : Device} -> UserDeviceLinear d => {inC, outC, len, kL, pad : Nat} ->
                Conv1DState inC outC len kL pad
                              (inC * len)
...
export
applyMaxPool2D : {0 d : Device} -> UserDeviceLinear d => {c, inH, inW, poolH, poolW, strH, strW : Nat} ->
                   MaxPool2DState c inH inW poolH poolW strH strW
                                    (c * (inH * inW))
                                    (c * (PoolOutDim inH poolH strH * PoolOutDim inW poolW strW))
                                    d g ->
...
export
applyMaxPool2DBatched : {0 d : Device} -> UserDeviceLinear d => {c, inH, inW, poolH, poolW, strH, strW : Nat} -> {b : Nat} ->
                          MaxPool2DState c inH inW poolH poolW strH strW
                                           (c * (inH * inW))
                                           (c * (PoolOutDim inH poolH strH * PoolOutDim inW poolW strW))
                                           d g ->
...
export
applyAvgPool2D : {0 d : Device} -> UserDeviceLinear d => {c, inH, inW, poolH, poolW, strH, strW : Nat} ->
                   AvgPool2DState c inH inW poolH poolW strH strW
                                    (c * (inH * inW))
                                    (c * (PoolOutDim inH poolH strH * PoolOutDim inW poolW strW))
                                    d g ->
...
export
applyMaxPool1D : {0 d : Device} -> UserDeviceLinear d => {c, len, poolK, str : Nat} ->
                   MaxPool1DState c len poolK str
...
export
applyAvgPool1D : {0 d : Device} -> UserDeviceLinear d => {c, len, poolK, str : Nat} ->
                   AvgPool1DState c len poolK str
                                    (c * len) (c * PoolOutDim len poolK str) d g ->
                   TVec (c * len) d g ->
                   TVec (c * PoolOutDim len poolK str) d g
