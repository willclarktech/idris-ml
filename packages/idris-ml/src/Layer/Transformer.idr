...
export
applyTransformer : {0 d : Device} -> UserDeviceTape d => UserDeviceCore d => RuntimeDType dt => Compatible d dt => {seqLen, dModel, numHeads, headDim, numBlocks, vocabSize : Nat} ->
...
mkLinearVec : UserDeviceTape d => RuntimeDType dt => Compatible d dt => {i, o : Nat} -> (n : Nat) -> String -> IO (Vect n (LinearState i o d dt WithGrad))
...
mkBlock : UserDeviceTape d => RuntimeDType dt => Compatible d dt => {dModel, numHeads, headDim : Nat} ->
            (paramPrefix : String) ->
...
mkBlocks : UserDeviceTape d => RuntimeDType dt => Compatible d dt => {dModel, numHeads, headDim : Nat} ->
...
export
transformerLayer :
  UserDeviceTape d => RuntimeDType dt => Compatible d dt =>
  {seqLen, dModel, numHeads, headDim, numBlocks, vocabSize : Nat} ->
