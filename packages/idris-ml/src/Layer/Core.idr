...
public export
interface LayerLike (l : Nat -> Nat -> (0 _ : Type) -> (0 _ : GradMode) -> Type) where
  ...
  applyVar : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} ->
              l i o d g -> Tensor [i] d g -> (l i o d g, Tensor [o] d g)

  ||| Auto-naming prefix (e.g. "llv2" for Linear).
  layerPrefix : {0 d : Type} -> {0 g : GradMode} -> {i, o : Nat} -> l i o d g -> String
  layerPrefix _ = ""
  ...
  resetState : {0 d : Type} -> {0 g : GradMode} -> {i, o : Nat} -> l i o d g -> l i o d g
  resetState = id
  ...
  applyVarBatch : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} -> {b : Nat} ->
  ...
  freezeLayer : {0 d : Type} -> {0 g : GradMode} -> {i, o : Nat} ->
  ...
  unfreezeLayer : {0 d : Type} -> {i, o : Nat} ->
...
public export
data AnyLayer : Nat -> Nat -> (0 _ : Type) -> (0 _ : GradMode) -> Type where
  MkAnyLayer : (l : Nat -> Nat -> (0 _ : Type) -> (0 _ : GradMode) -> Type) -> LayerLike l =>
                 l i o d g -> AnyLayer i o d g

export
applyVarAny : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} ->
...
export
applyVarBatchAny : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} -> {b : Nat} ->
...
export
freezeAnyLayer : {0 d : Type} -> {0 g : GradMode} -> {i, o : Nat} ->
...
export
unfreezeAnyLayer : {0 d : Type} -> {i, o : Nat} ->
...
public export
data Network : (i : Nat) -> (hs : List Nat) -> (o : Nat) -> (0 _ : Type) -> (0 _ : GradMode) -> Type where
...
export
forwardVar : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
...
export
freezeNetwork : {0 d : Type} -> {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
...
export
unfreezeNetwork : {0 d : Type} -> {i, o : Nat} -> {hs : List Nat} ->
...
export
resetNetwork : {0 d : Type} -> {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
...
||| interface default.
export
forwardVarBatch : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} -> {b : Nat} ->
                   {hs : List Nat} ->
...
export
forwardVarTraced : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
                   (label : String) ->
...
forwardVarTraced label net input = go 0 net input
  where
    ...
    go : {0 d : Type} -> UserDeviceCore d => {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
         Nat ->
         Network i hs o d g -> Tensor [i] d g ->
         IO (Network i hs o d g, Tensor [o] d g)
