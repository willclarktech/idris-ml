...
interface LayerLike (l : Nat -> Nat -> (0 _ : Device) -> (0 _ : GradMode) -> Type) where
  ...
  applyVar : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} ->
  ...
  applyVarBatch : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} -> {b : Nat} ->
...
export
applyVarAny : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} ->
...
export
applyVarBatchAny : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} -> {b : Nat} ->
...
export
forwardVar : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
...
||| interface default.
export
forwardVarBatch : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} -> {b : Nat} ->
                   {hs : List Nat} ->
...
export
forwardVarTraced : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
                   (label : String) ->
...
forwardVarTraced label net input = go 0 net input
  where
    ...
    go : {0 d : Device} -> UserDeviceLinear d => {0 g : GradMode} -> {i, o : Nat} -> {hs : List Nat} ->
         Nat ->
         Network i hs o d g -> Tensor [i] d g ->
         IO (Network i hs o d g, Tensor [o] d g)
