...
int compute_bcast_shape(Tensor* a, Tensor* b, int* r_shape, int* r_rank, int* r_numel) {
...
void compute_bcast_strides(Tensor* a, int r_rank, int* r_shape, int* out_strides) {
	int a_rank = a->rank;
	int natural[MAX_BCAST_RANK];
	int s = 1;
	for (int k = a_rank - 1; k >= 0; k--) {
		natural[k] = s;
		s *= a->shape[k];
	}
