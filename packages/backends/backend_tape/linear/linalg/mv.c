...
static TensorHandle tensor_mv_f32(TensorHandle hmat, TensorHandle hvec) {
	...
	for (int i = 0; i < m; i++) {
		float s = 0;
		for (int j = 0; j < n; j++)
			s += ((float*)mat->data)[i * n + j] * ((float*)vec->data)[j];
		out_data[i] = s;
	}
#endif
	Tensor* r =
	    make_tensor_arena_f32(out_data, m, out_shape, 1, mat->requires_grad || vec->requires_grad);
	if (r->requires_grad) {
		...
		for (int j = 0; j < n; j++)
			meta->x_vals[j] = (double)((float*)vec->data)[j];
		e->op_meta = meta;
	}
	return r;
}
...
TensorHandle tensor_mv(TensorHandle hmat, TensorHandle hvec) {
	...
	for (int i = 0; i < m; i++) {
		double s = 0;
		for (int j = 0; j < n; j++)
			s += ((double*)mat->data)[i * n + j] * ((double*)vec->data)[j];
		out_data[i] = s;
	}
#endif
	Tensor* r =
	    make_tensor_arena(out_data, m, out_shape, 1, mat->requires_grad || vec->requires_grad);
	if (r->requires_grad) {
		TapeEntry* e = tape_append(OP_MV, r, mat, vec, 0);
		MvMeta* meta = arena_alloc(sizeof(MvMeta));
		meta->m = m;
