...
#include <string.h>
#ifdef __APPLE__
#include <Accelerate/Accelerate.h> // IWYU pragma: keep — umbrella; provides cblas_*+vDSP_*+Cblas*
#endif
#include "../../tape.h"
#include "../../arena.h"
#include "../../tensor.h"
...
static TensorHandle tensor_linear_f32(TensorHandle hW, TensorHandle hx, TensorHandle hbias) {
	...
	for (int i = 0; i < m; i++) {
		float s = 0;
		for (int j = 0; j < n; j++)
			s += ((float*)W->data)[i * n + j] * ((float*)x->data)[j];
		out_data[i] = s;
	}
#endif
	if (bias) {
...
TensorHandle tensor_linear(TensorHandle hW, TensorHandle hx, TensorHandle hbias) {
	...
	Tensor* x = (Tensor*)hx;
	Tensor* bias = (Tensor*)hbias;
	if (W->dtype_tag == DT_F32 || x->dtype_tag == DT_F32 || (bias && bias->dtype_tag == DT_F32)) {
		if (W->dtype_tag != x->dtype_tag || (bias && bias->dtype_tag != W->dtype_tag))
			tape_abort_mixed_dtype("tensor_linear");
	...
	for (int i = 0; i < m; i++) {
		double s = 0;
		for (int j = 0; j < n; j++)
			s += ((double*)W->data)[i * n + j] * ((double*)x->data)[j];
		out_data[i] = s;
	}
#endif
	if (bias) {
	...
	if (b && b->requires_grad) {
		...
		if (a->dtype_tag == DT_F32) {
			...
			for (int jj = 0; jj < n_l; jj++) {
				double s = 0;
				for (int ii = 0; ii < m_l; ii++)
					s += ((double*)a->data)[ii * n_l + jj] * tape_grad_load_d(r, ii);
				tape_grad_add_d(b, jj, s);
			}
#endif
		}
