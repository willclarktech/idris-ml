/* Criterion suite for linear.c — tape-only F32 coverage top-ups.
 *
 * Covers the F32 zero-dim (n==0) bias-only branch of tensor_linear_f32:
 * when the inner dim collapses the matmul drops out and the output is the
 * (float) bias copied through. F32-tagged tensors store as float, so
 * readbacks use TEST_TOL_RELAXED. Tape-only (streamed dtag-14 path), so the
 * whole body is wrapped in #ifdef BACKEND_TAPE.
 */
