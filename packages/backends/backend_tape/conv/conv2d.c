...
TensorHandle tensor_conv2d(TensorHandle hinput, TensorHandle hkernel, TensorHandle hbias, int padH,
	...
	for (int oc = 0; oc < outC; oc++) {
		for (int oh = 0; oh < oH; oh++) {
			for (int ow = 0; ow < oW; ow++) {
						...
						for (int kw = 0; kw < kW; kw++) {
							...
							if (ih >= 0 && ih < H && iw >= 0 && iw < W) {
								val += tape_load_d(input, ic * H * W + ih * W + iw) *
								       tape_load_d(kernel, oc * inC * kH * kW + ic * kH * kW +
								                               kh * kW + kw);
							}
						}
					}
				...
				if (is_f32)
					((float*)out)[oc * oH * oW + oh * oW + ow] = (float)val;
				else
					((double*)out)[oc * oH * oW + oh * oW + ow] = val;
			}
		}
	}
	...
	Tensor* r;
	if (is_f32)
		r = make_tensor_arena_f32((float*)out, out_numel, out_shape, 3, rg);
	else {
							...
							for (int kw = 0; kw < kW; kw++) {
								...
								if (ih >= 0 && ih < HH && iw >= 0 && iw < WW)
									tape_grad_add_d(a, ic * HH * WW + ih * WW + iw,
									                dout * tape_load_d(b, oc * inC * kH * kW +
									                                          ic * kH * kW +
									                                          kh * kW + kw));
							}
				}
