/* Coverage suite for tape `tensor_conv1d` — arms the base test_conv1d.c
 * leaves uncovered: non-NULL bias (forward + backward), padding > 0 (the
 * il-out-of-range skip branch in both passes), stride > 1, and the
 * multi-channel (inC>1, outC>1) inner loops.
 *
 * Every oracle below is computed by hand from the definition
 *   out[oc, ol] = bias[oc] + sum_{ic,kl} input[ic, ol*stride - pad + kl]
 *                                        * kernel[oc, ic, kl]
 * with the sum-loss backward giving d_out = all ones.
 *
 * tensor_create (the base creator) COPIES its data buffer, so stack
 * arrays are safe here (mirrors test_conv1d.c); no hcopy needed.
 */
