...
TensorHandle tensor_max_pool2d(TensorHandle hinput, int kH, int kW, int strideH, int strideW) {
	...
	for (int c = 0; c < C; c++) {
		for (int oh = 0; oh < oH; oh++) {
			for (int ow = 0; ow < oW; ow++) {
				...
				if (is_f32)
					((float*)out_buf)[out_idx] = (float)best;
				else
					((double*)out_buf)[out_idx] = best;
	...
	Tensor* r;
	if (is_f32)
		r = make_tensor_arena_f32((float*)out_buf, out_numel, out_shape, 3, input->requires_grad);
	else {
		r = make_tensor((double*)out_buf, out_shape, 3, input->requires_grad);
		free(out_buf);
	}
