			...
			for (int ow = 0; ow < oW; ow++) {
				...
				for (int kh = 0; kh < kH; kh++)
					for (int kw = 0; kw < kW; kw++)
						s += tape_load_d(input,
						                 c * H * W + (oh * strideH + kh) * W + ow * strideW + kw);
				double v = s * scale;
				if (is_f32)
					((float*)out)[c * oH * oW + oh * oW + ow] = (float)v;
				else
					((double*)out)[c * oH * oW + oh * oW + ow] = v;
			}
	Tensor* r;
	if (is_f32)
		r = make_tensor_arena_f32((float*)out, numel, out_shape, 3, input->requires_grad);
	else {
		r = make_tensor((double*)out, out_shape, 3, input->requires_grad);
		free(out);
	}
	if (r->requires_grad) {
				...
				for (int ow = 0; ow < meta->oW; ow++)
					for (int kh = 0; kh < meta->kH; kh++)
						for (int kw = 0; kw < meta->kW; kw++)
							tape_grad_add_d(
							    a,
							    c * meta->H * meta->W + (oh * meta->strH + kh) * meta->W +
							        ow * meta->strW + kw,
							    tape_grad_load_d(r, c * meta->oH * meta->oW + oh * meta->oW + ow) *
							        scale);
	}
}
