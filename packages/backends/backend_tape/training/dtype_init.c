...
static double normal01(void) {
    if (bm_cached_has) { bm_cached_has = 0; return bm_cached_z1; }
    double u1 = uniform01();
    double u2 = uniform01();
...
static double* fill_normal_buf(int n, double mean, double std) {
    ...
    for (int i = 0; i < n; i++) buf[i] = mean + std * normal01();
    return buf;
}

static double* fill_const_buf(int n, double value) {
    double* buf = malloc((size_t)n * sizeof(double));
    for (int i = 0; i < n; i++) buf[i] = value;
    return buf;
}
...
TensorHandle tape_create_param_2d_normal_dtag(int rows, int cols, double mean, double std, int dtag) {
    return tape_create_param_2d_dtag(rows, cols, fill_normal_buf(rows * cols, mean, std), dtag);
}

TensorHandle tape_create_param_3d_normal_dtag(int d0, int d1, int d2, double mean, double std, int dtag) {
    return tape_create_param_3d_dtag(d0, d1, d2, fill_normal_buf(d0 * d1 * d2, mean, std), dtag);
}

TensorHandle tape_create_param_4d_normal_dtag(int d0, int d1, int d2, int d3, double mean, double std, int dtag) {
    return tape_create_param_4d_dtag(d0, d1, d2, d3, fill_normal_buf(d0 * d1 * d2 * d3, mean, std), dtag);
}
...
TensorHandle tape_create_param_4d_const_dtag(int d0, int d1, int d2, int d3, double value, int dtag) {
    return tape_create_param_4d_dtag(d0, d1, d2, d3, fill_const_buf(d0 * d1 * d2 * d3, value), dtag);
}
