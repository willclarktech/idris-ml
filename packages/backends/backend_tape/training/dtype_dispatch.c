...
static TensorHandle tape_arena_f32_from_doubles(int* shape, int rank, double* data, int rg) {
	int numel = 1;
	for (int i = 0; i < rank; i++)
		numel *= shape[i];
	float* arena_d = arena_alloc(numel * sizeof(float));
	for (int i = 0; i < numel; i++)
		arena_d[i] = (float)data[i];
	free(data);
...
static TensorHandle tape_persistent_f32_from_doubles(int* shape, int rank, double* data, int rg) {
	int numel = 1;
	for (int i = 0; i < rank; i++)
		numel *= shape[i];
...
TensorHandle tape_create_1d_dtag(int n, double* data, int rg, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {n};
		return tape_arena_f32_from_doubles(s, 1, data, rg);
	}
...
TensorHandle tape_create_2d_dtag(int rows, int cols, double* data, int rg, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {rows, cols};
		return tape_arena_f32_from_doubles(s, 2, data, rg);
	}
...
TensorHandle tape_create_param_1d_dtag(int n, double* data, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {n};
		return tape_persistent_f32_from_doubles(s, 1, data, /*rg=*/1);
	}
...
TensorHandle tape_create_param_2d_dtag(int rows, int cols, double* data, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {rows, cols};
		return tape_persistent_f32_from_doubles(s, 2, data, /*rg=*/1);
	}
...
TensorHandle tape_create_param_3d_dtag(int d0, int d1, int d2, double* data, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {d0, d1, d2};
		return tape_persistent_f32_from_doubles(s, 3, data, /*rg=*/1);
	}
...
TensorHandle tape_create_param_4d_dtag(int d0, int d1, int d2, int d3, double* data, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {d0, d1, d2, d3};
		return tape_persistent_f32_from_doubles(s, 4, data, /*rg=*/1);
	}
...
TensorHandle tape_create_state_1d_dtag(int n, double* data, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {n};
		return tape_persistent_f32_from_doubles(s, 1, data, /*rg=*/0);
	}
...
TensorHandle tape_create_state_2d_dtag(int rows, int cols, double* data, int dtag) {
	...
	if (dtag == 14) {
		int s[] = {rows, cols};
		return tape_persistent_f32_from_doubles(s, 2, data, /*rg=*/0);
	}
	...
	if (tag == DT_F32) {
		if (s->rank == 0) {
			...
			for (int i = 0; i < s->numel; i++)
				arena_d[i] = (float)tape_load_d(s, i);
