...
static void tape_backward_cosine_sim(TapeEntry* e) {
	...
	if (a && a->rank == 2 && b && b->rank == 2) {
		...
		for (int ii = 0; ii < n_cs; ii++) {
			double anorm2 = 0;
			for (int j = 0; j < w_cs; j++) {
				double v = tape_load_d(a, ii * w_cs + j);
				anorm2 += v * v;
			}
			double anorm = sqrt(anorm2) + 1e-8;
			...
			for (int j = 0; j < w_cs; j++) {
				tape_grad_add_d(a, ii * w_cs + j,
				                g * (tape_load_d(b, j) / (anorm * bnorm) -
				                     cos_val * tape_load_d(a, ii * w_cs + j) / (anorm2 + 1e-10)));
			}
		}
		...
		if (b->requires_grad) {
			...
			for (int ii = 0; ii < n_cs; ii++) {
				...
				for (int j = 0; j < w_cs; j++) {
					tape_grad_add_d(b, j,
					                g * (tape_load_d(a, ii * w_cs + j) / (anorm * bnorm) -
					                     cos_val * tape_load_d(b, j) / (bnorm2 + 1e-10)));
				}
			}
		}
	}
