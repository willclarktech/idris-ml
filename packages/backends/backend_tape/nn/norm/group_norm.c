...
TensorHandle tensor_group_norm(TensorHandle hinput, TensorHandle hgamma, TensorHandle hbeta,
	...
	int out_shape[] = {n};
	void* out = is_f32 ? (void*)arena_alloc(n * sizeof(float)) : (void*)calloc(n, sizeof(double));
	for (int g = 0; g < numGroups; g++) {
		double mean = 0;
		int base = g * groupSize;
		for (int j = 0; j < groupSize; j++)
			mean += tape_load_d(input, base + j);
		mean /= groupSize;
		double var = 0;
		for (int j = 0; j < groupSize; j++) {
	...
	Tensor* r;
	if (is_f32)
		r = make_tensor_arena_f32((float*)out, n, out_shape, 1,
		                          input->requires_grad || gamma->requires_grad);
	else {
		r = make_tensor((double*)out, out_shape, 1, input->requires_grad || gamma->requires_grad);
		free(out);
	}
