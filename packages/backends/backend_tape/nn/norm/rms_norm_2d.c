...
TensorHandle tensor_rms_norm_2d(TensorHandle h, TensorHandle hweight, double eps) {
	...
	if (t->dtype_tag != weight->dtype_tag) tape_abort_mixed_dtype("tensor_rms_norm_2d");
...
static void tape_backward_rms_norm_2d(TapeEntry* e) {
	...
	if (meta->weight->requires_grad) {
		...
		for (int j = 0; j < nn; j++) {
			double dw = 0;
			for (int i = 0; i < mm; i++)
				dw += tape_grad_load_d(r, i * nn + j) * meta->x_hat[i * nn + j];
			tape_grad_add_d(meta->weight, j, dw);
		}
	}
	if (a && a->requires_grad) {
		...
		for (int i = 0; i < mm; i++) {
			...
			for (int j = 0; j < nn; j++) {
				double dxhat = tape_grad_load_d(r, i * nn + j) * tape_load_d(meta->weight, j);
