...
Test(ternary_pack, roundtrip_length_12) {
	...
	cr_assert_eq(produced, 3, "ternary_pack should produce ceil(12/4) = 3 bytes (got %d)",
	             produced);
	 ...
	 */
	cr_assert_eq(packed[0], 0x71, "byte 0 expected 0x71 for slots {+1, 0, -1, +1}; got 0x%02X",
	             packed[0]);
	cr_assert_eq(packed[1], 0x43, "byte 1 expected 0x43 for slots {-1, 0, 0, +1}; got 0x%02X",
	             packed[1]);
	cr_assert_eq(packed[2], 0x1F, "byte 2 expected 0x1F for slots {-1, -1, +1, 0}; got 0x%02X",
	             packed[2]);
	...
	for (int i = 0; i < 12; i++) {
		cr_assert_eq(unpacked[i], values[i], "round-trip mismatch at index %d: expected %d, got %d",
...
Test(ternary_pack, roundtrip_length_5_padding) {
	...
	cr_assert_eq(produced, 2, "ternary_pack length=5 should produce ceil(5/4) = 2 bytes (got %d)",
	             produced);
	...
	for (int i = 0; i < 5; i++)
		cr_assert_eq(unpacked[i], values[i],
		             "non-multiple round-trip mismatch at index %d: expected %d, got %d", i,
		             (int)values[i], (int)unpacked[i]);
}
