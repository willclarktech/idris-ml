...
TensorHandle tensor_softmax_3d(TensorHandle h) {
	...
	for (int i = 0; i < total_rows; i++) {
		...
		for (int j = 0; j < n; j++) {
			if (is_f32)
				((float*)data)[i * n + j] /= (float)sum;
			else
				((double*)data)[i * n + j] /= sum;
		}
	}
	int shape[] = {B, m, n};
	Tensor* r;
	if (is_f32)
		r = make_tensor_arena_f32((float*)data, t->numel, shape, 3, t->requires_grad);
	else {
		r = make_tensor((double*)data, shape, 3, t->requires_grad);
		free(data);
	}
