...
Test(linear_linalg_dot, forward_backward) {
    ...
    for (int i = 0; i < 3; i++) {
        cr_assert_float_eq(param_grad_item_at(0, i), bd[i], 1e-12,
            "d_a[%d] should be b[%d]=%.1f (got %.6f)", i, i, bd[i], param_grad_item_at(0, i));
        cr_assert_float_eq(param_grad_item_at(1, i), ad[i], 1e-12,
            "d_b[%d] should be a[%d]=%.1f (got %.6f)", i, i, ad[i], param_grad_item_at(1, i));
    }
}
...
Test(linear_linalg_outer, forward_backward) {
    ...
    for (int i = 0; i < 6; i++) cr_assert_float_eq(out[i], expected[i], 1e-12);
    TensorHandle loss = tensor_sum(r);
    tensor_backward(loss);
    /* d_a[i] = sum_j b[j] = 3+4+5 = 12; d_b[j] = sum_i a[i] = 1+2 = 3 */
    cr_assert_float_eq(param_grad_item_at(0, 0), 12.0, 1e-12,
        "d_a[0] should be 12 (got %.6f)", param_grad_item_at(0, 0));
    cr_assert_float_eq(param_grad_item_at(0, 1), 12.0, 1e-12);
    for (int j = 0; j < 3; j++)
        cr_assert_float_eq(param_grad_item_at(1, j), 3.0, 1e-12,
            "d_b[%d] should be 3 (got %.6f)", j, param_grad_item_at(1, j));
}
