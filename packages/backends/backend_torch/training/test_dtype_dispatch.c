/* torch-only Criterion suites for the dtype-dispatch + streamed-shell
 * creator layer.
 *
 * Covers the torch-specific dtag routing in training/dtype_dispatch.cpp
 * and the shared streamed-shell wrappers in shared/training/dtype_streamed.c:
 *
 *   - suite torch_training_dtype_dispatch: the F32 (dtag 14) explicit-suffix
 *     path and the BF16/F16/I32/Bool generic `default` arms across nd/scalar/
 *     1d/2d non-param, param (ranks 1-4), and state (ranks 1-2) creators, plus
 *     streamed cast and the BF16 requires_grad branch.
 *   - suite dtype_dispatch_cov2: the remaining FLOAT dtag arms — the F64
 *     (dtag 15) `case 15:` explicit-suffix wrappers, make_param_leaf's F64
 *     no-cast fast path, the rg != 0 requires_grad branches of the 1d/2d
 *     f32/f64 creators, create_scalar_dt's floating requires_grad branch, and
 *     st_for_dtag's case 15 via a cast-to-F64.
 *
 * Routing goes through the active port (tensor_create_*_streamed), so a
 * streamed call exercises BOTH the shared shell AND the torch dispatcher in
 * one shot. stream_tag is ignored by torch (single-stream); pass 0. torch CPU
 * base dtype is F64: F64 roundtrips are exact (TEST_TOL_TIGHT), F32 roundtrips
 * of whole/dyadic values assert at 1e-5, BF16/F16 at TEST_TOL_RELAXED,
 * integer dtypes exact.
 */
...
/* ============================================================================
 * Suite dtype_dispatch_cov2 — the remaining FLOAT dtag arms:
 *   - F64 (dtag 15) `case 15:` explicit-suffix wrappers + scalar.
 *   - make_param_leaf's F64 no-cast/no-move fast path.
 *   - the rg != 0 requires_grad branch of the 1d/2d f32/f64 creators.
 *   - create_scalar_dt's `rg && is_floating` requires_grad branch.
 *   - st_for_dtag's case 15 (Float64) via a cast-to-F64.
 * I32/Bool param-leaf arms are NOT tested — make_param_leaf forces
 * requires_grad_(true) which c10-throws on integer dtypes (abort guard
 * excluded from coverage by policy).
 * ==========================================================================*/
