 ...
 */
static void bench_attention_sdpa(int seq, int numHeads, int numKvHeads, int headDim, int isCausal,
                                 int iters) {
	...
	printf("sdpa seq=%d H=%d Hkv=%d d=%d%s:\t%.3f ms  (%d iters)\n", seq, numHeads, numKvHeads,
	       headDim, isCausal ? " causal" : "", elapsed, iters);
	tensor_free(q);
	tensor_free(k);
	tensor_free(v);
}
