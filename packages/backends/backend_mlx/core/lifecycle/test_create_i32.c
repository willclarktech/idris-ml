...
Test(mlx_core_lifecycle_create_i32, dispatch_1d_roundtrip) {
    ...
    cr_assert_str_eq(tensor_dtype_name(x), "I32",
        "after I32 dispatch, dtype should be I32 (got %s)",
        tensor_dtype_name(x));
    for (int i = 0; i < 5; i++) {
        double v = tensor_item_1d(x, i);
        cr_assert_float_eq(v, xd[i], 0.0,
            "I32 roundtrip [%d]: expected %.0f got %.6f",
            i, xd[i], v);
    }
}
...
Test(mlx_core_lifecycle_create_i32, dispatch_2d_roundtrip_via_to_doubles) {
    ...
    for (int i = 0; i < 6; i++) {
        cr_assert_float_eq(buf[i], xd[i], 0.0,
            "I32 to_doubles [%d]: expected %.0f got %.6f",
            i, xd[i], buf[i]);
    }
}
...
Test(mlx_core_lifecycle_create_i32, dispatch_param_1d_roundtrip) {
    ...
    for (int i = 0; i < 3; i++) {
        cr_assert_float_eq(buf[i], xd[i], 0.0,
            "I32 param to_doubles [%d]: expected %.0f got %.6f",
            i, xd[i], buf[i]);
    }
}
