...
static std::pair<mx::Shape, mx::Shape>
multi_axis_slice_bounds(const mx::Shape& shape, int dim, int start, int len) {
    ...
    return { mx::Shape(start_v.begin(), start_v.end()),
             mx::Shape(stop_v.begin(),  stop_v.end()) };
}
