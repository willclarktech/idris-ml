...
extern "C" TensorHandle tensor_tile_2d_mlx_streamed(TensorHandle h, int rep0, int rep1,
	...
	if (t->requires_grad) {
		...
		meta[0] = rep0;
		meta[1] = rep1;
		int const idx = tape_append(OP_TILE_2D, r, t, nullptr, 0);
		if (idx >= 0) {
			tape[idx].meta = meta;
		} else {
			// GCOVR_EXCL_START — only a grad-tracked tile inside no_grad reaches this (tape_append
