...
enum : std::uint8_t {
	...
	OP_RMS_NORM_2D,  /* row-wise RMS normalization (HF LlamaRMSNorm); meta carries weight pool idx +
	                    eps */
	...
	OP_SOFTMAX_XENT_2D, /* -scale * sum(target * log_softmax(input, rows)); scale in scalar_arg */
