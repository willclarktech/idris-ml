...
extern "C" OptimizerHandle optimizer_create_adam(double lr, double beta1, double beta2, double eps) {
    auto opt = new Optimizer();
...
extern "C" void optimizer_free(OptimizerHandle h) { delete (Optimizer*)h; }
extern "C" void optimizer_zero_grad(OptimizerHandle h) { (void)h; param_zero_all_grads(); }
...
static void adam_step_compile(Optimizer* opt, int np) {
    ...
    for (int j = 0; j < n; j++) ins.push_back(opt->m_bufs[active_idx[j]]);
    for (int j = 0; j < n; j++) ins.push_back(opt->v_bufs[active_idx[j]]);
    for (int j = 0; j < n; j++) {
        int i = active_idx[j];
        double lr = opt->lr;
