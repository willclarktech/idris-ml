...
/* ----------------------------------------------------------------------
   ...
   so a -> a - lr (for g > 0).
   ---------------------------------------------------------------------- */
/* DISABLED (won't-fix, mlx-upstream): the g++ shim lets mx::compile RUN and
   the assertions pass, but mlx's Metal-allocator/device static teardown
   crashes the forked child on exit ("crashed during teardown"). Clearing
   our cache AND mx::detail::compile_clear_cache() both failed — the throw is
   inside libmlx's shutdown (see backend_mlx/init.cpp + TODO.md). Compile-path
   coverage is a principled exclusion until mlx fixes the teardown race. */
