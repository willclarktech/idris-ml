...
byo_tensor byo_tensor_create(double* data, int* shape, int rank, int requires_grad) {
    (void)shape; (void)rank; (void)requires_grad;
...
byo_tensor byo_tensor_neg(byo_tensor a)    { fprintf(stderr,"[byo] tensor_neg\n");    return make_scalar(a ? -(*a) : 0.0); }
byo_tensor byo_tensor_abs(byo_tensor a)    { fprintf(stderr,"[byo] tensor_abs\n");    return make_scalar(a && *a < 0 ? -(*a) : (a ? *a : 0.0)); }
byo_tensor byo_tensor_exp(byo_tensor a)    { fprintf(stderr,"[byo] tensor_exp\n");    return make_scalar(0.0); }
byo_tensor byo_tensor_log(byo_tensor a)    { fprintf(stderr,"[byo] tensor_log\n");    return make_scalar(0.0); }
byo_tensor byo_tensor_sqrt(byo_tensor a)   { fprintf(stderr,"[byo] tensor_sqrt\n");   return make_scalar(0.0); }
byo_tensor byo_tensor_pow(byo_tensor a, byo_tensor b) { (void)a; (void)b; fprintf(stderr,"[byo] tensor_pow\n");    return make_scalar(0.0); }
byo_tensor byo_tensor_sigmoid(byo_tensor a){ fprintf(stderr,"[byo] tensor_sigmoid\n");return make_scalar(0.5); }
byo_tensor byo_tensor_tanh(byo_tensor a)   { fprintf(stderr,"[byo] tensor_tanh\n");   return make_scalar(0.0); }
