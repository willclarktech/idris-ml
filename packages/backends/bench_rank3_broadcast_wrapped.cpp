...
static double bench_strided(torch::Device d, bool use_f32) {
	...
	TensorHandle x = use_f32 ? tensor_create_f32_torch(x_data.data(), x_shape, 3, 0)
	                         : tensor_create_f64_torch(x_data.data(), x_shape, 3, 0);
	TensorHandle tbl = use_f32 ? tensor_create_f32_torch(tbl_data.data(), tbl_shape, 2, 0)
...
static double bench_contig(torch::Device d, bool use_f32) {
	...
	TensorHandle x = use_f32 ? tensor_create_f32_torch(x_data.data(), x_shape, 3, 0)
	                         : tensor_create_f64_torch(x_data.data(), x_shape, 3, 0);
	TensorHandle cos = use_f32 ? tensor_create_f32_torch(cos_data.data(), cos_shape, 3, 0)
