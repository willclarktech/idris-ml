...
countRights : Source -> Nat -> Nat -> Nat
...
countRights seed (S k) acc =
  let (st, _) = initFL True seed
      ...
      (_, seed') = Random.Source.next seed
...
tests =
  [ check "non-slippery step right" $
      let (st, _) = initFL False (Seeded 42)
          r  = flStep st 2
  ...
  , check "slippery has ~1/3 rightward from start" $
      ...
      let rights = countRights (Seeded 7) 300 0
