...
Test(nn_layers, narrow_layernorm_cat_gradient) {
	...
	{
		double w_copy[4];
		memcpy(w_copy, w_data, sizeof(w_data));
		param_clear();
		...
		double fp = tensor_item(tensor_sum(tensor_add(
		    tensor_reshape(
		        tensor_cat2(
		            tensor_reshape(
		                tensor_mm(tensor_reshape(
		                              tensor_narrow(tensor_reshape(np, flat_shape, 1), 0, 0, 6),
		                              seq_shape, 2),
		                          tensor_transpose_2d(wp)),
		                of_shape, 1),
		            tensor_reshape(
		                tensor_mm(tensor_reshape(
		                              tensor_narrow(tensor_reshape(np, flat_shape, 1), 0, 6, 6),
		                              seq_shape, 2),
		                          tensor_transpose_2d(wp)),
		                of_shape, 1)),
		        x_shape, 2),
		...
		double fm = tensor_item(tensor_sum(tensor_add(
		    tensor_reshape(
		        tensor_cat2(
		            tensor_reshape(
		                tensor_mm(tensor_reshape(
		                              tensor_narrow(tensor_reshape(nm, flat_shape, 1), 0, 0, 6),
		                              seq_shape, 2),
		                          tensor_transpose_2d(wm)),
		                of_shape, 1),
		            tensor_reshape(
		                tensor_mm(tensor_reshape(
		                              tensor_narrow(tensor_reshape(nm, flat_shape, 1), 0, 6, 6),
		                              seq_shape, 2),
		                          tensor_transpose_2d(wm)),
		                of_shape, 1)),
		        x_shape, 2),
