...
Test(training_optimizer_adamw_foreach, matches_scalar_on_256_elem_param) {
	...
	double wdata1[256];
	for (int i = 0; i < N; i++)
		wdata1[i] = (double)(i % 7) * 0.1 - 0.3;
	TensorHandle W1 = tensor_create_param_2d_f64(16, 16, hcopy(wdata1, N));
	param_register("W", W1);
	...
	double wdata2[256];
	for (int i = 0; i < N; i++)
		wdata2[i] = (double)(i % 7) * 0.1 - 0.3;
	TensorHandle W2 = tensor_create_param_2d_f64(16, 16, hcopy(wdata2, N));
	param_register("W", W2);
...
Test(training_optimizer_fused_mv, multi_epoch_loss_decreases) {
	...
	for (int ep = 0; ep < 5; ep++) {
		...
		if (ep > 0)
			cr_assert(lv < prev_loss + 0.01, "loss decreasing (ep=%d lv=%.4f prev=%.4f)", ep, lv,
			          prev_loss);
		prev_loss = lv;
		tensor_backward(loss);
		optimizer_step(sgd);
