...
static void create_scalar_params(const char* prefix, int count, double init, TensorHandle* out) {
...
static TensorHandle stack_scalars(TensorHandle* handles, int n) {
	...
	for (int i = 0; i < n; i++)
		tensor_ptr_array_set_return(arr, i, handles[i]);
...
Test(ntm_timestep, lstm_addressing_backward_no_nans) {
	...
	for (int i = 0; i < INP; i++)
		input[i] = tensor_create_scalar(i < 4 ? 1.0 : 0.0, 0);
	...
	for (int i = 0; i < M; i++)
		lstm_inp[i] = read_out[i];
	for (int i = 0; i < INP; i++)
		lstm_inp[M + i] = input[i];
	...
	TensorHandle gamma_v =
	    tensor_add_scalar(tensor_log(tensor_add_scalar(tensor_exp(gamma_raw), 1.0)), 1.0);
