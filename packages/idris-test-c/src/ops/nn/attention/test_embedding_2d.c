...
Test(nn_attention_embedding_2d, shape_is_rank_2) {
	...
	cr_assert_eq(tensor_dim(r), 2, "tensor_embedding_2d output should be rank 2, got %d",
	             tensor_dim(r));
	cr_assert_eq(tensor_size(r, 0), 2, "output dim 0 expected 2 (n), got %d", tensor_size(r, 0));
	cr_assert_eq(tensor_size(r, 1), 3, "output dim 1 expected 3 (embedDim), got %d",
	             tensor_size(r, 1));
}
...
Test(nn_attention_embedding_2d, matches_decomposed_chain) {
	...
	for (int k = 0; k < 15; k++) {
		cr_assert_float_eq(twoD_buf[k], flat_buf[k], TEST_TOL_RELAXED,
		                   "embedding_2d[%d]=%.6f vs flat[%d]=%.6f", k, twoD_buf[k], k,
		                   flat_buf[k]);
	}
}

Test(nn_attention_embedding_2d, embedding) {
    param_clear();
