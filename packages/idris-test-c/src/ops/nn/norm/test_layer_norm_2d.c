...
Test(nn_norm_layer_norm_2d, layer_norm_2d_backward) {
	...
	{
		double d_copy[6];
		memcpy(d_copy, data, sizeof(data));
		param_clear();
		d_copy[0] = data[0] + eps;
	...
	{
		double g_copy[3];
		memcpy(g_copy, gamma_data, sizeof(gamma_data));
		param_clear();
	...
	{
		double b_copy[3];
		memcpy(b_copy, beta_data, sizeof(beta_data));
		param_clear();
