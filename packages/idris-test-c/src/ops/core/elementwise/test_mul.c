...
Test(core_elementwise_mul, backward_scalar_swapped_grads) {
	...
	TensorHandle c = tensor_mul(a, b);
	tensor_backward(c);
	cr_assert_float_eq(param_grad_item_at(0, 0), 4.0, 1e-12, "d(a*b)/da should be b=4.0 (got %.6f)",
	                   param_grad_item_at(0, 0));
	cr_assert_float_eq(param_grad_item_at(1, 0), 3.0, 1e-12, "d(a*b)/db should be a=3.0 (got %.6f)",
...
Test(core_elementwise_mul, backward_vector_swap) {
	...
	for (int i = 0; i < 3; i++) {
		cr_assert_float_eq(param_grad_item_at(0, i), bd[i], 1e-12, "d_a[%d] should be b[%d]=%.1f",
		                   i, i, bd[i]);
		cr_assert_float_eq(param_grad_item_at(1, i), ad[i], 1e-12, "d_b[%d] should be a[%d]=%.1f",
		                   i, i, ad[i]);
	}
}
