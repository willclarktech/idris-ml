...
Test(dtype_scaffolding, unified_dtag_dispatch) {
	...
	ASSERT_TRUE("unified create_1d dtag=1 -> F64", strcmp(tensor_dtype_name(t), "F64") == 0);
	double out[3];
	...
	ASSERT_TRUE("unified cast dtag=1 -> F64", strcmp(tensor_dtype_name(c), "F64") == 0);
	double cout[3];
	...
	ASSERT_TRUE("unified create_1d dtag=6 -> I32", strcmp(tensor_dtype_name(ti), "I32") == 0);
#endif
	param_clear();
}
...
Test(dtype_scaffolding, runtime_dtype_tag_layout) {
	...
	for (size_t i = 0; i < sizeof(universal_cases) / sizeof(universal_cases[0]); i++) {
		...
		char label[128];
		snprintf(label, sizeof(label), "dtag=%d expected %s got %s", universal_cases[i].dtag,
		         universal_cases[i].name, got);
	...
	struct {
	    ...
	    {1, "Bool"}, {4, "U8"},   {8, "I8"},   {9, "I16"},
	    {10, "I32"}, {11, "I64"}, {13, "F16"}, {17, "BF16"},
	};
	for (size_t i = 0; i < sizeof(inference_cases) / sizeof(inference_cases[0]); i++) {
		...
		snprintf(label, sizeof(label), "dtag=%d expected %s got %s", inference_cases[i].dtag,
		         expected, got);
