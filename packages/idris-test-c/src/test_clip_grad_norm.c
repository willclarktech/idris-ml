...
Test(training_optimizer_clip_grad_norm, clip_when_norm_exceeds_max) {
    ...
    double xd[] = {3.0, 4.0};
    double yd[] = {12.0};
    TensorHandle x = tensor_create_param_1d_f64(2, heap_copy(xd, 2));
    TensorHandle y = tensor_create_param_1d_f64(1, heap_copy(yd, 1));
    param_register("x", x);
    param_register("y", y);
    ...
    cr_assert_float_eq(tensor_item(loss), 84.5, CLIP_TOL,
        "loss should be 84.5 (got %.9f)", tensor_item(loss));
    ...
    cr_assert_float_eq(param_grad_item_at(1, 0), 12.0 * s, CLIP_TOL,
        "post-clip y.grad[0] should be 12*5/13 (got %.9f)", param_grad_item_at(1, 0));
}
