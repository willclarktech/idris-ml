...
### Arena must size-check a *recycled* `next` chunk (not just allocate-larger on grow)

Bumping `ARENA_INIT_SIZE` to 4MB only covered the case where the **new** chunk is freshly allocated (the grow path correctly sizes `cap = max(bytes, ARENA_INIT_SIZE)`). But `arena_alloc`'s *other* grow branch **recycles a pre-allocated `next` chunk** after `arena_reset` rewinds the bump pointer to the head — and it did so without checking that the recycled chunk was large enough. Any single allocation bigger than a recycled chunk then overran it on the caller's `memcpy` in `make_tensor`. MNIST's conv1 output (`[64,16,24,24]` = 4.5MB) is the only intermediate in any example that exceeds the 4MB default chunk, so it was the only example that tripped it — manifesting downstream (after the next `arena_reset`) as a garbage `e->result` (an overwritten arena-allocated `Tensor` header) in `tensor_backward`'s tape walk, i.e. the campaign-2026-06-17 P1 "invalid memory reference". Fix: reuse `next` only when `next->cap >= bytes`; otherwise splice a correctly-sized chunk in front of it (preserving the undersized tail so `arena_reset`/`arena_free_all` still walk the whole chain). Regression guard: `backend_tape/test_arena_recycle.c` (deterministic, drives the exact head→next chain through the public `tensor_create` API), pinned originally with ASan (`make test-unit-c-asan-tape` → heap-buffer-overflow at `arena.c` `make_tensor`).

### Collated batch tensors are arena intermediates — don't hold one across training

A batch from `DataStream.batched`'s `.next` (catAllTensors + reshape) is **arena-allocated**, so it dies at the next `optimizer_step` → `arena_reset`. Pre-fetching an eval/validation batch *before* the training loop and using it *after* (the original `Example/Mnist.idr`) is a use-after-free: thousands of training-step resets clobber its arena memory, and the eval forward reads a dangling header (garbage `t->data`). The fix is the same discipline `Transformer`/`Gpt` already follow — **construct/fetch eval inputs after training, inside the eval block** (params survive resets because they're malloc'd, not arena'd; data does not). The linear-model type discipline does *not* catch this — tensors are deliberately unrestricted (reverse-mode AD shares them), so this lifetime rule is by-convention.
...
**Update (2026-07-27 audit — the thresholds above no longer reproduce standalone).** On the pack toolchain (`0.8.0-b2d2cf40d`, nightly-260604) against the current `Nn` surface, all of these elaborate in under a second: `Dropout 512/2304/9216`, `Dropout (4 * 576)`, a `Seq` chain with existential hidden dim 2304 (`linear {i=8} {o=2304}` → `dropout` → `linear {i=2304} {o=8}`), and even a raw `Tensor [4 * o]` signature with a free variable. The original observations were on the V1 `Layer` surface + an earlier toolchain, and no specific upstream fix was identified — so treat the hazard as *unconfirmed on the current stack* rather than gone. The *pattern-compilation* sibling below is NOT fixed — it still reproduces on the same nightly.

**Update (2026-07-28 — the in-tree removal experiment passed).** `Ml/Nn/Lstm.idr`'s four multiplicative fields were temporarily de-aliased to raw `Tensor [4 * o, i]` / `Tensor [4 * o]` forms (the exact shape the `TMat` doc comment names as the hanging one) and a full `make install` ran clean: 4 m 15 s wall / 637 MB max RSS across all five packages, indistinguishable from the aliased-baseline rebuild. The routing rule is therefore **advisory, not load-bearing, on the pinned toolchain**: new code MAY write raw multiplicative shape literals; the `TVec`/`TMat` aliases stay in-tree because 95 use sites of churn buys nothing (and `Test/pos/SingleUseCompiles.idr` uses them for a *different* reason — the `[2]`/`[3]` list-literal `(::)` ambiguity). If a hang recurs, capture the module + toolchain pin here and re-tighten the rule.
...
**Passing an argument through is not sufficient if it is the same constant at every call site.** `dropoutSeed 0` threads its argument into the FFI call, but the whole expression is identical everywhere it appears, so it is still one subexpression to eliminate. A dummy argument only helps when it actually varies. When the varying value does not exist (a pure RNG tick), type the FFI `PrimIO a` and bind it with `<-`: the `%World` token orders the call and nothing can fold it. That is what `Ml.Nn.Dropout`'s `prim__dropoutSeed` does — a pure-typed version would collapse to one mask for the whole run.
...
HF transformers 5.9.0's `BitNetDeserialize.convert` (`src/transformers/integrations/bitnet.py`) threads `target_dtype = weight.dtype` (= `uint8`) through `unpack_weights` instead of the model's `torch.bfloat16` dtype. After loading `microsoft/bitnet-b1.58-2B-4T` on CPU each `AutoBitLinear`'s `weight` is left as `uint8 [out, in]` with ternary codes wrap-encoded (`-1 → 255`, `0 → 0`, `+1 → 1`). The next forward dies with `RuntimeError: BFloat16 != unsigned char`. Workaround used in `packages/idris-transformers/scripts/save_oracle_bitnet.py`: walk every module whose class name is `AutoBitLinear` and rewrite the weight in place via `mod.weight.data = mod.weight.data.view(torch.int8).to(torch.bfloat16)`. Viewing the U8 buffer as I8 reinterprets `255` as `-1`, recovering `{-1, 0, +1}`. 210 BitLinears patched on the 2B-4T model (30 layers × 7 = 5 attn projections + 2 MLP projections). Drop the workaround once the upstream bug fix lands (file an issue against `huggingface/transformers` if not already tracked). Also: HF's BitNet quantizer's `validate_environment` raises `ImportError` without `accelerate` — `uv add accelerate` in `packages/pytorch`.
...
`packages/pytorch/.venv/.../transformers/integrations/bitnet.py` has two `nn.Linear`-replacement classes, both named with "BitLinear" prefixes but with subtly different forward algebra:
...
The streamed-creation path (`tensor_create_*_streamed` → `tensor_create_*_{f32,f64}` in `backend_torch/core/lifecycle/create.cpp`) routes new tensors to `g_torch_target_device` — the process-wide pin set at dylib load from `TORCH_DEVICE`. Any *new* tensor creator that calls `torch::empty` / `torch::zeros` / `torch::from_blob` *without* honouring this global silently lands on CPU regardless of `TORCH_DEVICE=mps|cuda`. When the activation tensors land correctly on MPS but the new tensor stays on CPU, the next op that consumes both crashes with `c10::Error: Expected <something>.is_mps() to be true, but got false` (e.g. `addmv_out_mps_impl`). The bitlinear backend had this exact bug — `tensor_create_ternary_packed_2d` + `tensor_create_ternary_from_hf_packed_2d` left the int8 W on CPU, and the make-path BitNet inference run aborted mid-forward on `at::matmul(W_cast, x)`. Fix template: build the buffer on CPU (per-byte loops are fast there), then `if (g_torch_target_device.type() != c10::DeviceType::CPU) { t = t.to(g_torch_target_device); }` before `from_tensor_persistent`. Direct-shell runs without `TORCH_DEVICE` set won't show the bug — the global stays at `at::kCPU` and everything lands on CPU; the gate is `BACKEND=torch TORCH_DEVICE=mps make example-<your-new-op>` or equivalent. (Also: the standard `param_load` path goes through the dtype-streamed creators, so it Just Works — this gotcha only bites bespoke creators that exist outside that pipeline, like the packed-ternary ABI for BitNet weights.)
...
### An unstamped CI ttc-cache restore wedges permanently (idris-fmt, 2026-07-30)

CI's ttc cache (`packages/*/build`, restored by `.github/actions/setup-idris-ml`) is only sound because two steps cooperate: `git restore-mtime` back-dates every source to its last-touching commit's date so unchanged modules stay older than the cached `.ttc`, and "Reconcile source mtimes" then `touch`es whatever changed between the restored tree's stamp (`build/.cache-commit`) and HEAD. Miss the second half and the first half is actively harmful — a back-dated source looks older than a stale `.ttc` built from an earlier version of itself, so idris2 keeps the stale one.

That is what happened to `packages/idris-fmt/build`: a tree restored without a `.cache-commit` hit the old `exit 0` ("cold or pre-v4 tree — nothing to reconcile") path, so `Format/Render.ttc` predating `377ed205`'s `formatWith` / `isFormattedWith` survived. `Main.idr` (which did rebuild) then failed with `Undefined name isFormattedWith` — the tell is a build log that numbers only the modules it rebuilt (`1/7: Building Format.Ipkg`, `7/7: Building Main`) and silently skips the rest. The state is self-sustaining: every failed run re-saved the same poisoned tree under a fresh `<sha>` stamp, so the reconcile step had nothing to touch on the next run. It stayed red across three CI runs (2026-07-26, 07-27, 07-30).

Two fixes, both needed. A missing `.cache-commit` now forces a full rebuild rather than trusting the tree (cold trees have no `.ttc` to skip, so it costs nothing there). And clearing an already-wedged tree needs the cache-key version bump the restore step documents — `v4` → `v5` — since the poisoned entries have a valid stamp and will otherwise be trusted forever.
...
`tensor_softplus` uses the numerically stable form `max(0,x) + log(1 + exp(-|x|))` to avoid `log(1+exp(x))` overflowing in float32 for `x > ~88`. The naive form is C^∞ smooth and gives correct `sigmoid(x)` backward via mlx's `vjp` everywhere; the stable form is non-smooth at exactly `x=0` (both `max` and `abs` have subgradient ambiguity). At that boundary point mlx's `vjp` picks the 0 subgradient for each → `d_softplus(0)` returns 0 instead of the expected `sigmoid(0) = 0.5`. All non-boundary inputs (|x| > 0 by even a float ulp) get the correct derivative. The permanent fix (commit `46e57ad`): keep the stable composite in the forward but register a custom replay closure that reconstructs the **smooth** form `log(1 + exp(x))` (differentiable everywhere, vjp = `sigmoid(x)`).

`tensor_bce_with_logits` had the **same** kink (its stable form is `max(p,0) - p*y + log(1+exp(-|p|))`, with the `max`/`abs` non-smoothness at `p=0`); the decomposed-primitive path gave `d/dp = -1.0` at the kink instead of `sigmoid(0)-y = -0.5`. Fixed identically: one `OP_BCE_WITH_LOGITS` tape entry whose replay uses the smooth `mean(log(1+exp(p)) - p*y)`, so its vjp gives `(sigmoid(p)-y)/n` everywhere. Tape (closed form) and torch (libtorch fused) already agreed; this aligns mlx at the kink. See `backend_mlx/nn/loss/bce_with_logits.cpp` and the `backward_at_kink` probe in `test_bce_with_logits.c`.
