...
**Open follow-up — the husk leaks handles on long grad-mode runs.** Husks stay in `all_tensors` until their Idris wrap drains, but the grad-mode epoch bracket (`tensor_epoch_end`) does *not* `forceMajorGc + drainManagedHandles` (only `withNoGrad` does), so during pure-training loops Chez never GCs the dead wraps → the guardian never drains them → husks at `rc==1` accumulate (~1650/epoch on ntm-copy, linear). Harmless at smoke-test scale (50 MB at 30 epochs) but unbounded for 10000-epoch convergence runs. Tracked in `TODO.md`; the masking allocate-throw is gone so the leak is now observable. Candidate fixes: GC+drain in the mlx epoch bracket (perf cost), or generation-tagged wraps (v3 layout) that let `delete`-sweep be ABA-safe again.
