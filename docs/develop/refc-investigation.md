...
## Addendum (2026-07-28): the wrapped-handle ABI re-blocked RefC

The Supervised retry recommended above was attempted on the pack nightly
(`0.8.0-b2d2cf40d`, GMP from the nix store, no `refc_shims.c`, no
`IDRIS2_PREFIX` override so the pack support tree resolves). It fails during
refc codegen, before any C is compiled or linked:

```text
Error: INTERNAL ERROR: [refc] FFI not found for Ml_Executor_Tape_Transfer_prim__toHostTape
```

The cause is a change that post-dates this document's "Scheme FFI Elimination
(completed)" section: the **wrapped-handle ABI** (`docs/develop/tensor-lifecycle.md`)
reintroduced Scheme-only FFI at scale. Every Tensor-handle-touching `%foreign`
now binds to a `"scheme:(lambda ...)"` wrapper that unwraps the
`#(tensor-handle-v2 tag raw)` Chez vector on arguments and wraps + registers
returns with the `idris-tensor-guardian`. As of 2026-07-28 that is **505
scheme-only declarations across 13 files** (`Ml/Executor/{Tape,Torch,Mlx}/
{Core,Nn,Linear,Transfer}.idr` + `Ml/Tensor/Handle.idr`), all in Supervised's
dependency cone and none carrying a `C:` fallback specifier. The "Zero
Scheme-specific FFI remaining" claim earlier in this file is therefore
historical only.

**What adoption now requires** (in order of real cost):
1. **Dual-specifier emission** — the wrappers are generated by
   `scripts/codegen/ffi-convert-to-scheme.py` from the `ffi_manifest` package,
   so emitting a paired `C:<sym>,libidrisml` specifier per declaration is
   mechanical (Idris picks the matching convention per codegen; RefC would use
   raw pointers directly, which is fine — the Chez-vector wrap exists for the
   Chez GC, not for C).
2. **A refc-side lifecycle** — the real scope. On Chez the wrap + guardian
   registration IS the retain/release mechanism (mlx refcounts through it).
   RefC has no guardian; the raw-pointer path needs its own release story
   (RefC finalizers, or explicit frees at the `withGenFree`-style brackets)
   before anything beyond tape-leak-everything mode is correct.
3. Then the runtime-symbol question from the 2026-07-27 addendum (re-derived
   shims against the current `_datatypes.h`) becomes reachable at link time.
