...
typedef struct { int tag; int refCounter; } Value_header;
typedef struct { Value_header header; double d; } Value_Double;
typedef struct { Value_header header; mpz_t i; } Value_Integer;
typedef struct { Value_header header; char *str; } Value_String;
...
void* idris2_cast_string_to_Integer(void* x) {
    ...
    mpz_init(v->i);
    if (mpz_set_str(v->i, ((Value_String*)x)->str, 10) != 0)
        mpz_set_si(v->i, 0);
    return v;
}
