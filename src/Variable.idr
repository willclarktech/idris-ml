...
-- Also assigns a dense integer pid_id and sets it in C-side tape_pid_ids array.
-- Lazily initializes pid-to-id hash table on first call.
%foreign "scheme:(lambda (idx pid) (when (not (top-level-bound? 'pid-to-id)) (begin (set-top-level-value! 'pid-to-id (make-hashtable string-hash string=?)) (set-top-level-value! 'next-pid-id 0) (set-top-level-value! 'tape-pid-id-set! (foreign-procedure \"tape_pid_id_set\" (int int) void)))) (vector-set! (top-level-value 'tape-pids) idx pid) (let* ((ht (top-level-value 'pid-to-id)) (existing (hashtable-ref ht pid #f))) (if existing ((top-level-value 'tape-pid-id-set!) idx existing) (let ((new-id (top-level-value 'next-pid-id))) (set-top-level-value! 'next-pid-id (+ new-id 1)) (hashtable-set! ht pid new-id) ((top-level-value 'tape-pid-id-set!) idx new-id)))) idx)"
...
%foreign "scheme:(lambda (g sz) (let ((n ((foreign-procedure \"walk_backward_ext\" (void* int void* void* void* void*) int) g sz (top-level-value 'tape-tags-fp) (top-level-value 'tape-arg1-fp) (top-level-value 'tape-arg2-fp) (top-level-value 'tape-vals-fp)))) ((foreign-procedure \"ext_meta_reset\" () void)) ((foreign-procedure \"arena_reset\" () void)) ((foreign-procedure \"tape_pid_ids_reset\" (int) void) sz) (set-top-level-value! 'tape-size 0) (set-top-level-value! 'tape-gen (+ (top-level-value 'tape-gen) 1)) n))"
...
-- Also sets C-side pid_ids for each entry (looked up from pid-to-id table).
%foreign "scheme:(lambda (wbuf count) (if (= (vector-ref wbuf 3) (top-level-value 'tape-gen)) (vector-ref wbuf 2) (let* ((cbuf (vector-ref wbuf 0)) (pids (vector-ref wbuf 1)) (start (top-level-value 'tape-size)) (end (+ start count))) ((top-level-value 'tape-ensure-cap!) (- end 1)) (let ((tags-fp (top-level-value 'tape-tags-fp)) (vals-fp (top-level-value 'tape-vals-fp)) (pidv (top-level-value 'tape-pids)) (ht (if (top-level-bound? 'pid-to-id) (top-level-value 'pid-to-id) #f)) (set-pid-id! (if (top-level-bound? 'tape-pid-id-set!) (top-level-value 'tape-pid-id-set!) #f))) (do ((k 0 (+ k 1))) ((= k count)) (let ((idx (+ start k))) (foreign-set! 'integer-32 tags-fp (* idx 4) 0) (foreign-set! 'double vals-fp (* idx 8) (foreign-ref 'double cbuf (* k 8))) (vector-set! pidv idx (vector-ref pids k)) (when (and ht set-pid-id!) (let ((id (hashtable-ref ht (vector-ref pids k) -1))) (when (>= id 0) (set-pid-id! idx id))))))) (set-top-level-value! 'tape-size end) (vector-set! wbuf 2 start) (vector-set! wbuf 3 (top-level-value 'tape-gen)) start)))"
