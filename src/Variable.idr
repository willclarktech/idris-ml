...
||| Stub: weight buffer not used in libtorch backend.
export
matrixVectorMultiplyVarBuf : {m, n : Nat} -> AnyPtr -> Vector n Variable -> Vector m Variable
matrixVectorMultiplyVarBuf {m} wBuf (VTensor xs) =
  VTensor $ replicate m (STensor (fromDouble 0.0))

||| Matrix-vector multiply with fused bias using persistent buffers.
||| Stub: layers should use matrixVectorMultiplyVar + bias instead.
export
...
----------------------------------------------------------------------
-- NTM Buffer Variant Stubs
----------------------------------------------------------------------

||| Batch cosine similarity with buffer output. Stub.
export
batchCosineSimilarityVarBufBufOut : {n, w : Nat} -> Variable -> AnyPtr -> Vector w Variable -> (AnyPtr, Int)
batchCosineSimilarityVarBufBufOut {n} {w} beta memBuf keyVector = (prim__createScalar 0.0 0, 0)

||| Interpolation write with NTM memory buffer. Stub.
export
interpolationWriteVarBuf : {n, w : Nat} -> Vector n Variable -> AnyPtr -> Vector w Variable -> AnyPtr
interpolationWriteVarBuf {n} {w} weights memBuf addVector = memBuf

||| Softmax with buffer output. Stub.
export
softmaxVarBufIO : {n : Nat} -> (AnyPtr, Int) -> (AnyPtr, Int)
softmaxVarBufIO {n} x = x

||| Interpolate with buffer I/O. Stub.
export
interpolateVarBufIO : {n : Nat} -> Variable -> (AnyPtr, Int) -> Vector n Variable -> (AnyPtr, Int)
interpolateVarBufIO {n} g contentBuf prevWeights = contentBuf

||| Softmax returning raw buffer. Stub.
export
softmaxVarBufOut : {n : Nat} -> Vector n Variable -> (AnyPtr, Int)
softmaxVarBufOut {n} xs = (prim__createScalar 0.0 0, 0)

||| Shift with buffer I/O. Stub.
export
shiftVarBufIO : {n : Nat} -> (AnyPtr, Int) -> (AnyPtr, Int) -> (AnyPtr, Int)
shiftVarBufIO {n} inputBuf kernelBuf = inputBuf

||| Focus from buffer. Stub: returns zeros.
export
focusVarFromBuf : {n : Nat} -> Variable -> (AnyPtr, Int) -> Vector n Variable
focusVarFromBuf {n} gamma buf = VTensor $ replicate n (STensor (fromDouble 0.0))

||| Read operation from NTM memory buffer. Stub: returns zeros.
export
readOpVarBuf : {n, w : Nat} -> Vector n Variable -> AnyPtr -> Vector w Variable
readOpVarBuf {n} {w} weights memBuf = VTensor $ replicate w (STensor (fromDouble 0.0))
...
lstmCellVarBuf : {o : Nat} -> Vector (4 * o) Variable -> Vector (4 * o) Variable
              ...
              -> (Vector o Variable, Vector o Variable)
lstmCellVarBuf {o} mulIW mulRW bBuf cs = (cs, cs)  -- stub

||| Matrix-vector multiply returning raw buffer + tape start.
||| Stub: returns dummy buffer pair.
export
matrixVectorMultiplyVarBufOut : {m, n : Nat} -> AnyPtr -> Vector n Variable -> (AnyPtr, Int)
matrixVectorMultiplyVarBufOut {m} {n} wBuf xs = (prim__createScalar 0.0 0, 0)

||| LSTM cell from raw buffers. Stub.
export
lstmCellVarFromBufs : {o : Nat}
                   ...
                   -> Vector o Variable
                   -> (Vector o Variable, Vector o Variable)
lstmCellVarFromBufs {o} mulIWBuf mulIWStart mulRWBuf mulRWStart bBuf cs = (cs, cs)

||| LSTM cell from raw buffers, extended (returns buffer info). Stub.
export
...
lstmCellVarFromBufsExt {o} mulIWBuf mulIWStart mulRWBuf mulRWStart bBuf cs =
  (cs, cs, prim__createScalar 0.0 0, 0)

||| FC matmul from buffer inputs. Stub.
export
...
||| Hybrid FC matmul from buffer + vector. Stub.
export
matrixVectorMultiplyVarBufBiasFromBufAndVec : {m, n1, n2 : Nat} -> AnyPtr -> AnyPtr
  -> AnyPtr -> Int -> Int -> Vector n2 Variable -> Vector m Variable
matrixVectorMultiplyVarBufBiasFromBufAndVec {m} {n1} {n2} wBuf bBuf srcBuf srcOff srcTapeStart vec =
  VTensor $ replicate m (STensor (fromDouble 0.0))
...
----------------------------------------------------------------------
-- Weight Buffer Stubs (backward compat — buffers removed in libtorch backend)
----------------------------------------------------------------------

-- These functions provided for backward compatibility with layer code
-- that uses weight buffers. In the libtorch backend, weights are stored
-- directly as tensors. The buffer functions are no-ops.

export
prim__weightBufAlloc : Int -> AnyPtr
prim__weightBufAlloc _ = prim__createScalar 0.0 0  -- stub

export
prim__weightBufApplyDeltas : AnyPtr -> AnyPtr -> AnyPtr
prim__weightBufApplyDeltas buf _ = buf  -- stub (no-op)

export
initWeightBufRow : AnyPtr -> Int -> Vect k (Scalar Variable) -> AnyPtr
initWeightBufRow wBuf _ [] = wBuf
initWeightBufRow wBuf off (STensor _ :: rest) = initWeightBufRow wBuf (off + 1) rest
...
syncWeightBufRow wBuf _ _ = wBuf  -- no-op

export
syncWeightBuf : AnyPtr -> Int -> {n : Nat} -> Vect m (Vector n Variable) -> AnyPtr
syncWeightBuf wBuf _ _ = wBuf  -- no-op

export
readWeightBufRow : AnyPtr -> Int -> Vect k (Scalar Variable) -> Vect k (Scalar Variable)
readWeightBufRow _ _ xs = xs  -- no-op (values already in Variables)

export
readWeightBuf : AnyPtr -> Int -> {n : Nat} -> Vect m (Vector n Variable) -> Vect m (Vector n Variable)
readWeightBuf _ _ rows = rows  -- no-op


----------------------------------------------------------------------
-- NTM Memory Buffer Stubs
----------------------------------------------------------------------

export
prim__ntmMemBufAlloc : Int -> Int -> AnyPtr
prim__ntmMemBufAlloc _ _ = prim__createScalar 0.0 0

export
initNtmMemBuf : AnyPtr -> Int -> {w : Nat} -> Vect n (Vector w Variable) -> AnyPtr
initNtmMemBuf mb _ _ = mb

export
syncNtmMemBuf : AnyPtr -> Int -> {w : Nat} -> Vect n (Vector w Variable) -> AnyPtr
syncNtmMemBuf mb _ _ = mb

export
readNtmMemBuf : AnyPtr -> Int -> {w : Nat} -> Vect n (Vector w Variable) -> Vect n (Vector w Variable)
readNtmMemBuf _ _ rows = rows

export
prim__ntmMemBufApplyDeltas : AnyPtr -> AnyPtr -> AnyPtr
prim__ntmMemBufApplyDeltas mb _ = mb

export
prim__ntmMemBufResetCache : AnyPtr -> AnyPtr
prim__ntmMemBufResetCache mb = mb

export
prim__ntmMemBufReset : AnyPtr -> AnyPtr
prim__ntmMemBufReset mb = mb

export
prim__ntmMemBufSnapshotInitial : AnyPtr -> AnyPtr
prim__ntmMemBufSnapshotInitial mb = mb

export
prim__ntmMemBufUpdateAfterWrite : AnyPtr -> AnyPtr -> AnyPtr
prim__ntmMemBufUpdateAfterWrite mb _ = mb


----------------------------------------------------------------------
-- Dense Optimizer Stubs
----------------------------------------------------------------------

-- Dense optimizer infrastructure removed — libtorch handles gradients.
-- These stubs allow existing code to compile during migration.
...
prim__denseAlloc _ = prim__createScalar 0.0 0

export
prim__denseZero : AnyPtr -> Int -> AnyPtr
prim__denseZero p _ = p
