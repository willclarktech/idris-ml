...
def _one_cycle_lr(base_lr: float, peak_ratio: float, div_final: float, pct_start: float, total_epochs: int, epoch: int) -> float:
    """One-cycle learning rate schedule matching Schedule.idr oneCycle."""
    warmup_epochs = int(pct_start * total_epochs)
    if epoch < warmup_epochs:
        # Linear warmup from base_lr/peak_ratio to base_lr
        lr_start = base_lr / peak_ratio
        return lr_start + (base_lr - lr_start) * epoch / max(warmup_epochs, 1)
    else:
        # Cosine annealing from base_lr to base_lr/div_final
